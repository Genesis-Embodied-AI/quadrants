...
`qd.init(...)` accepts a wide set of configuration options as keyword arguments; each is also reachable as an environment variable of the form `QD_<UPPERCASE_NAME>` (e.g. `QD_OFFLINE_CACHE=0`). This page describes the knobs that are commonly tuned in practice; the [full list](#all-options) at the bottom is generated from the option schema ([`tools/config_codegen/schema.py`](https://github.com/Genesis-Embodied-AI/quadrants/blob/main/tools/config_codegen/schema.py)), the single source of truth for every option's name, type, and default.
...
The table below is generated directly from the option schema (the same source that defines the defaults used at runtime), so it can never drift from the code. It lists every `qd.init` keyword, its type, and its default:
