#!/usr/bin/env python3
"""Builds the pre-compiled fatbin for the qd.checkpoint() IF-gate kernel.
...
def generate_header(fatbin_path: Path, out_path: Path) -> None:
    ...
    header_lines.append("};")
    header_lines.append("")
    header_lines.append(f"static const std::size_t kCheckpointGateKernelFatbinSize = {len(fatbin_data)};")
    header_lines.append("")
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        obj_path = Path(tmpdir) / "checkpoint_gate.o"
        fatbin_path = Path(tmpdir) / "checkpoint_gate.fatbin"
