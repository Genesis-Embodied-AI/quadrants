#!/usr/bin/env python3
"""Builds the pre-compiled fatbins for the qd.checkpoint() IF-gate kernel.

The output is a C header (checkpoint_gate_fatbin.h) containing one fatbin per build toolkit as byte arrays, which is
compiled into the quadrants binary. At runtime this lets us load the gate kernel without needing libcudadevrt.a on the
user's system. Mirrors the build pipeline for graph_do_while_cond.cu (see scripts/build_condition_kernel_fatbin.py).
...
def main() -> None:
    build_fatbin_header(
        ...
        base_name="kCheckpointGateKernelFatbin",
    )
