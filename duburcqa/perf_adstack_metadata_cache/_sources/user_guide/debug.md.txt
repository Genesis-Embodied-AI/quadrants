...
The adstack overflow check on reverse-mode autodiff runs always, on every backend, regardless of `debug`. A push past the per-stack capacity (set via `qd.init(ad_stack_size=...)` or per-alloca by `determine_ad_stack_size`) raises `QuadrantsAssertionError("[Aa]dstack overflow")` at the next Quadrants Python entry - kernel launch, host-side field / ndarray read, or explicit `qd.sync()`. The error message describes the two possible causes (untracked tensor mutation between launches, or sizer under-estimate) and the recovery flow; see [Autodiff -> What can go wrong](autodiff.md) for the full description.
