...
- `qd.amdgpu` currently wires up the Linux x64 ROCm path only. AMD's HIP SDK also ships on Windows and on some Linux ARM64 targets, but quadrants does not support them yet. AMDGPU codegen is pinned to wave64 on every target — CDNA (gfx9xx) defaults to wave64 already, and RDNA (gfx10/11/12) is forced into wave64 mode via `+wavefrontsize64,-wavefrontsize32` and the `oclc_wavefrontsize64_on.bc` libdevice variant. This means `qd.simt.subgroup` primitives on AMDGPU always operate over 64 lanes, whereas the GPU's hardware default would have been 32 lanes on RDNA.
