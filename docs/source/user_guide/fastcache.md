...
1. **If the kernel does not read or write a variable, it is entirely ignored by fastcache.** It will not cause fastcache to fail, nor emit a warning, nor emit an error.
   ...
   A `@property` of a `@qd.data_oriented` object that the kernel reads, e.g. `qd.static(cfg.n_rows)`, is keyed on its *value*: the members the property derives from are read by plain Python outside the kernel, so they are not recorded as read themselves. The property must therefore be deterministic. If its value has an unrecognised type, or its getter raises, fastcache is disabled for the call, as for any other read of an unrecognised type.
