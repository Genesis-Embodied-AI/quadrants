...
Here the value of `num_layers` is compiled into the kernel. If that value changes, a different kernel must be compiled. Mark these fields with `add_value_to_cache_key` so their values are included in the cache key:
