...
The same holds for a [`dataclasses.dataclass`](compound_types.md#dataclassesdataclass) passed as a typed parameter: the compiler flattens each declared field into its own kernel parameter, so `params.dt` inside the kernel is a parameter access, not capture. By default a primitive field is passed as a runtime scalar argument - only its type enters the fastcache key, not its value (to bake the value in instead, mark the field with `FIELD_METADATA_CACHE_VALUE`, a dataclass field-metadata flag that Quadrants exports) - and an ndarray field becomes a runtime external tensor. See [Compound-type cache keying](#compound-type-cache-keying) for the full keying rules, including the exact form of that annotation.
...
1. **If the kernel does not read or write a variable, it is entirely ignored by fastcache.** It will not cause fastcache to fail, nor emit a warning, nor emit an error.
   ...
   A `@property` of a `@qd.data_oriented` object that the kernel reads, e.g. `qd.static(cfg.n_rows)`, is keyed on its *value*: the members the property derives from are read by plain Python outside the kernel, so they are not recorded as read themselves. The property must therefore be a pure function of the object's state (no randomness, clock or global counters): fastcache evaluates it again when computing the key, and a value that differs from the one the kernel was compiled with would store the kernel under a key that does not describe it. If its value has an unrecognised type, or its getter raises, fastcache is disabled for the call, as for any other read of an unrecognised type.
