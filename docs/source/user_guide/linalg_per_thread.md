...
- **Shapes are fixed.** Calling any of these on a matrix outside the supported shapes raises an exception at trace time (e.g. `"SVD only supports 2×2 and 3×3 matrices."`). For shapes up to the cap they compile down to per-thread straight-line code; beyond the cap there is no fallback today.
