...
- **Inner reverse-mode loop with a complex bound at very large extent.** An arbitrarily large enclosing range works only when the inner trip count fits a fixed subset of expressions; other shapes cap at ~16 million enclosing iterations and raise `RuntimeError: ... iteration count ... exceeds the 16777216 guard` past that. Workaround: rewrite the trip count to stay within the supported subset, or shrink the enclosing loop below the threshold.
  - *Works at any enclosing-range size:* integer ndarray reads (single- or multi-axis, indexed by literal constants or enclosing loop variables), `arr.shape[k]` shape terms, literal-indexed field reads (`my_field[None]`, `my_field[k]` for a constant `k`), literal integer constants, and `+`, `-`, `*`, `max` of those.
  - *Caps at the threshold:* field reads indexed by an enclosing loop variable (`my_field[i]`), arithmetic-indexed reads (`arr[i // 2]`), or a ragged inner range whose own bound depends on the enclosing loop variable (`for j in range(arr[i])`).
