...
- **Inner reverse-mode loop with a complex bound at very large extent.** When the inner loop's trip count is a simple read from a 1D integer ndarray at the enclosing loop variable - `for i in range(arr.shape[0]): ... for _ in range(arr[i])` - the bound is evaluated in parallel and the enclosing range can be arbitrarily large. More complex trip-count expressions - multi-axis indexing like `arr[i, j]`, two different ndarrays in the same expression, non-integer reads, or arithmetic mixing more than one outer-loop variable - fall back to a serial evaluator that refuses to enumerate more than ~16 million iterations, raising `RuntimeError: ... iteration count ... exceeds the 16777216 guard`. Fix: simplify the trip count to a single integer 1D-ndarray read at the enclosing loop variable, or shrink the enclosing loop below the threshold.
