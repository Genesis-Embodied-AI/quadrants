...
The sized result is cached per task across launches and reused as long as nothing the formula reads has changed. Quadrants tracks invalidation automatically for writes that flow through a Quadrants API - kernel-side writes, `Ndarray.write` / `Ndarray.fill`, and field writes. Writes that bypass Quadrants (DLPack zero-copy mutations through a torch tensor sharing storage with a Quadrants ndarray, or raw pointer writes through a non-torch DLPack consumer) are NOT tracked: the next launch reuses the cached size, and if the bypass mutation grew the required capacity, the kernel overflows the adstack and raises a clear error at the next Quadrants Python entry. See [What can go wrong](#what-can-go-wrong) for the error contract and recovery flow.
...
- **Adstack overflow.** Surfaces as `QuadrantsAssertionError: Adstack overflow ...` at the next Quadrants Python entry (kernel launch, host-side field / ndarray read, or explicit `qd.sync()`). The error message names two possible causes:
  1. *Untracked tensor mutation between launches.* A tensor that backs a data-dependent loop bound was written to outside Quadrants's tracking - typically a DLPack zero-copy mutation through a torch tensor sharing storage with a Quadrants ndarray, or a raw pointer write through a non-torch DLPack consumer. Quadrants caches the adstack capacity computed at the previous launch; if such a write grew the required capacity, the next launch overflows. Fix: route the write through Quadrants APIs (`Ndarray.write` / `Ndarray.fill` / a kernel that writes the value), OR pass a generous `ad_stack_size=N` to `qd.init()` if a workload milestone genuinely grew capacity. Restart the iteration / training loop from a clean state.
  2. *Sizer under-estimated the bound (Quadrants bug).* On unusually intricate nested loops - typically deeply nested `for i in range(arr[...])` with cumulative-index arithmetic - the sizer can compute a bound that is mathematically tighter than the actual push count. Please file a bug with `QD_DUMP_IR=1` set so the kernel IR ships with the report. Workarounds, in order of convenience: shorten the innermost dynamic loop; precompute its worst-case trip into a scalar field the kernel only reads; split the inner section into its own `@qd.kernel`; pass `ad_stack_size=N` to `qd.init()` with `N` large enough to cover the real push count (bypasses the sizer).

  Kernel state may be inconsistent after an overflow; do not retry the same step without addressing the cause and restarting from a clean state.
...
- **Loop bounds backed by a mutated ndarray.** A reverse-mode kernel with `for i in range(n[j])` requires `n[j]` to hold the same value at the forward call and at `.grad()`. Writes between those two points that go through Quadrants APIs (`Ndarray.write` / `Ndarray.fill` / kernel writes) are tracked: the cached adstack capacity is recomputed automatically, the kernel runs against the new value, gradients are correct. Writes that bypass Quadrants's tracking (DLPack zero-copy, raw pointer writes through non-torch consumers) are NOT tracked. If such a write grows `n[j]` between the forward call and `.grad()`, the kernel overflows the cached adstack and raises the *Adstack overflow* error described above. The error names this cause as the most common possibility. The safe rule: populate loop-bound ndarrays through Quadrants APIs, or before the forward call and leave them untouched until `.grad()` returns. Tape-based eager AD like [PyTorch's autograd](https://pytorch.org/docs/stable/notes/autograd.html) is not affected, since the trip count is recorded as the forward runs and reused at backward time.
