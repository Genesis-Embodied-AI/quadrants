...
Returns the subgroup size in effect for the current launch as an `i32`. **For use inside `@qd.kernel` / `@qd.func` bodies only** — calling it from host Python raises, since the value is emitted into the kernel IR rather than computed in CPython. There is no host-side equivalent at the moment; host code that needs the same number has to hard-code per backend off `prog.config().arch` until a separate `qd.subgroup_size() -> int` API lands.
