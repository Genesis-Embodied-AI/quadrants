...
- `key` and `value` are scalar values held one-per-lane.  Both must support `<` and `==`; the supported scalar dtypes are the union of what `subgroup.shuffle` accepts (`i32`, `u32`, `f32`, `f64`, `i64`, `u64`). The dtypes of `key` and `value` do not have to match.
