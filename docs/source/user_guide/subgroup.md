...
CUDA shortcut: `all_true` / `any_true` lower to a single `__all_sync(0xFFFFFFFF, p)` / `__any_sync(0xFFFFFFFF, p)` (one `vote.all` / `vote.any` instruction). Every other backend uses a portable `shuffle_xor` butterfly with no branch in the emitted IR (the low-level intermediate representation the compiler generates from your kernel).
...
- **CUDA**: the value is the constant `32`, folded into your kernel as a literal - no runtime warp-size query, and no PTX instructions emitted for `group_size()` itself.
- **AMDGPU**: the value is `64`. Quadrants pins every AMDGPU target to wave64 (`+wavefrontsize64`, see [supported_systems](supported_systems.md)), so CDNA (gfx9xx, gfx940/942) keeps its native wave64 mode and RDNA (gfx10/11/12) - which would otherwise default to wave32 - is forced into wave64 too.
- **SPIR-V (Vulkan / Metal)**: probed once at device-creation time. Vulkan reads the subgroup width the device reports; Metal is always `32` (every shipping Apple / AMD / Intel Mac GPU is 32-wide). Two SPIR-V devices with different subgroup widths are compiled independently, so each gets code specialized to its own width.
...
- Built on a single `subgroup.ballot(head_flag != 0)` to locate the segment heads, followed by a segmented inclusive scan that is fully unrolled at compile time. The wave64-only case (`log2_size == 6`, reachable only on AMDGPU) adds no cost on wave32 backends: CUDA / Metal / Vulkan compile to exactly the same code as before wave64 was supported.
...
- Backend lowering:
  ...
  - **AMDGPU**: `llvm.amdgcn.ballot.i64` followed by `trunc to i32`, which packs lanes 0..31 into the result; on wave64, lanes 32..63's predicates are discarded by the truncate, matching the `n <= 32` contract. Costs nothing - it lowers to a single `v_cmp_*_e64` plus a low-half store.
