...
| Tensor members                      | `qd.field`, `qd.ndarray`              | `qd.field`                            | none (see note below)               |
...
`@qd.kernel(fastcache=True)` is supported on methods of `@qd.data_oriented` classes; see [Appendix — compound-type cache keying](fastcache.md#compound-type-cache-keying) for more information.
...
|---|:---:|:---:|:---:|:---:|:---:|:---:|
| `dataclasses.dataclass`         | yes | yes | yes | yes | no       | yes |
| `@qd.data_oriented`             | no  | yes | yes | yes | yes      | yes |
...
| `qd.Template`                   | value-driven walk of `vars(self)`                  | for `@qd.data_oriented` containers (and primitives) |
