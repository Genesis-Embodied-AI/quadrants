...
Device-wide algorithms operate on whole arrays — reductions, scans, radix sort, stream compaction, and reduce-by-key — built on the block and subgroup primitives one tier below. Each op has a friendly host (Python) entry that validates its inputs, sizes the work, and launches it standalone; that entry is the default choice. Most also ship a composable `@qd.func` variant (the `_func` suffix) you call at the **top level** of your own kernel, so the op is captured into the same kernel / graph as your surrounding phases instead of running as a separate launch. The `_func` forms take the live element count as a device-resident value plus a compile-time depth, so one captured graph replays for any count up to that depth's capacity. Every op requires caller-owned **scratch**, covered in the next section.
