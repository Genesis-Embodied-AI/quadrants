...
The agent first qualifies each modified existing file as hot/core using `git log` commit count, recent churn, distinct author count, and file size. If a host file is not hot, nothing in it is flagged. In hot files, only plausibly extractable blocks are flagged (clear feature/subsystem name, mostly self-contained, reasonable target file path). New methods added to an existing central class are explicitly in scope, since this is one of the most common shapes of the smell. Newly added files, refactors, small tweaks, and code tightly coupled to host internals are out of scope. The agent reports up to 5 violations, each annotated with the host file's hotness numbers (commits / authors / size). This check is delayed by 30 minutes, to avoid running repeatedly if multiple commits pushed with a short delay between each.
