...
- Known limitation: per-task artifacts are not yet counted toward `offline_cache_max_size_of_files`, nor evicted. Each artifact key includes the Quadrants version, so upgrading is always correct (old artifacts stop matching and are never reused), but those orphaned files are not reclaimed, so a workload that keeps producing new task keys can grow the directory. Folding this tier into size-based eviction is a planned follow-up.
