...
- On CUDA it also enables a **per-task artifact cache**: editing one task in a large kernel reuses the *other* tasks' compiled code across processes instead of recompiling the whole kernel.
