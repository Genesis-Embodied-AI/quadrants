...
- On CUDA and AMDGPU it also enables a **per-task artifact cache**: a kernel is compiled as several *tasks* (roughly one per parallel loop), so editing one task reuses the *other* tasks' compiled code across processes instead of recompiling the whole kernel. The persisted artifact is the backend's compiled code (PTX on CUDA, HSACO on AMDGPU); because that code is device-architecture-specific, the cache is scoped per GPU target (CUDA compute capability, AMDGPU gfx target), so switching GPUs never reuses another target's code.
