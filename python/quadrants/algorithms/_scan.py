...
def _device_exclusive_scan(inp, *, out, op, identity_value):
    ...
    if dtype not in _SUPPORTED_DTYPES:
        raise NotImplementedError(
            f"device exclusive scan dtype {dtype} not in first-land set "
    ...
    if B0 > scratch_capacity_u32():
        raise RuntimeError(
            f"device exclusive scan on N={N} needs >= {B0} u32 scratch "
            f"slots, but only {scratch_capacity_u32()} are configured. "
    ...
    _exclusive_scan_inplace_u32(scratch, 0, B0, identity_bits, op, dtype, B0)
...
def _scan_single_tile_input_to_out(
    ...
    """Fast path for ``N <= BLOCK_DIM``: one block reads the input tile,
