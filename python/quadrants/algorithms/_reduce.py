...
Implements ``qd.algorithms.device_reduce_{add,min,max}`` on top of the
block-tier ``block.reduce_{add,min,max}`` primitives. See the design doc at
...
- **First pass** reads the caller's input tensor (of the algorithm's
  ``dtype``) and writes per-block partials to the shared scratch field as
  ``u32`` via ``qd.bit_cast``.
- **Intermediate passes** (only needed when ``N`` is large enough to require
  more than two passes total — i.e. ``B0 > BLOCK_DIM``) read from one slice of
  scratch (``u32`` → ``dtype`` via ``qd.bit_cast``) and write to another
  slice (``dtype`` → ``u32`` via ``qd.bit_cast``).
- **Last pass** reduces to a single value and writes it directly to the
  caller's ``out`` tensor as ``dtype`` (no bit_cast on the write side).

A single generic kernel handles every pass; ``src_is_u32`` and ``dst_is_u32``
are compile-time template flags selecting between the bit_cast and direct-read
/ direct-write paths.

The shared scratch field is owned by ``quadrants._scratch`` (see that
module). For first land, the default 1 MB capacity covers reductions up to
~64M elements at ``BLOCK_DIM=256``.

The reduce monoid identity (e.g. ``+inf`` for ``min`` over ``f32``,
``2**31 - 1`` for ``min`` over ``i32``) is passed to the kernel as its raw
4-byte bit pattern in a ``u32`` runtime arg, then ``qd.bit_cast``-ed to
``dtype`` inside the kernel. This bypasses the ``default_ip`` overflow check
that ``cast(literal, dtype)`` would otherwise hit on the wider unsigned
identities, and keeps ``identity`` out of the kernel template key (one fewer
axis of cache fragmentation).
"""
...
Chosen as a portable default: a multiple of every supported subgroup size
(32 on CUDA / Vulkan-on-NV / Metal, 64 on AMDGPU), and small enough to fit
comfortably in shared memory budgets across backends. Re-tune (128 / 512) once
...
def _identity_bits(value, dtype) -> int:
    ...
    Used to ferry monoid identities (e.g. ``+inf`` for ``min`` over ``f32``,
    ``2**31 - 1`` for ``min`` over ``i32``) into the reduce kernel as a
    ``u32`` runtime arg, sidestepping the ``default_ip`` overflow check that
...
def _reduce_pass(
    ...
    Reads ``src[src_off : src_off + n_valid]`` (out-of-range threads use the
    monoid identity so the per-block aggregate is correct), reduces in tiles of
    ``BLOCK_DIM`` via ``block.reduce(op, dtype)``, and writes per-block partials
    to ``dst[dst_off : dst_off + ceil(n_valid / BLOCK_DIM)]``.

    Template flags ``src_is_u32`` / ``dst_is_u32`` switch between the
    ``qd.bit_cast``-on-access scratch path and the direct-read / direct-write
    path used for the caller's input on the first pass and ``out`` on the last
    pass respectively.

    ``op`` is one of ``subgroup._bin_{add,min,max}`` (or any user-supplied
    associative ``@qd.func`` binary op of the right shape). ``identity_bits``
    is the monoid identity for ``op`` over ``dtype``, reinterpreted as a
    ``u32`` (see ``_identity_bits`` on the host side).
    """
...
def _plan_levels(N: int):
    ...
    - ``sizes[k]`` is the number of valid input elements consumed at level
      ``k``; ``sizes[-1] == 1``.
...
def _src_off(k: int, dst_offsets):
    """Source offset for pass ``k`` (k >= 1): equals the dst offset that pass
    ``k - 1`` wrote to."""
...
def _trivial_write_input(input: template(), out: template()):  # pylint: disable=redefined-builtin
    """N == 1 path: copy input[0] to out[0]. Two-element kernel keeps the host
    driver loop-free for the trivial case."""
    for _ in range(1):
        out[0] = input[0]
...
def _trivial_write_identity(out: template(), identity_bits: u32, dtype: template()):
    ...
    Quadrants doesn't support 0-shape tensors today, so this path is currently
    unreachable from a caller — left in place for defensiveness against future
    0-length support.
    """
    for _ in range(1):
...
def device_reduce_add(input, *, out):  # pylint: disable=redefined-builtin
    ...
    Args:
        ...
        out: 1-element tensor of the same dtype as ``input``. Caller-supplied
            so the call is fully asynchronous — no implicit device→host sync.
            To get a Python scalar, do ``out.to_numpy()[0]`` explicitly after
            this call.

    The implementation is a two-or-more-pass tree reduction built on
    ``block.reduce_add``. Scratch is drawn from the quadrants-level shared
    scratch field; no per-call allocation. See the design doc at
    ``perso_hugh/doc/qipc/qipc_device_algos_design.md`` for the recursion
    plan and the ``bit_cast``-into-scratch scheme.
    """
...
def device_reduce_min(input, identity, *, out):  # pylint: disable=redefined-builtin
    ...
    Args:
        ...
        identity: the monoid identity for ``min`` over ``input.dtype`` —
            ...
            in the dtype. For ``f32``, that's ``+inf`` (``math.inf``). For
            ``i32``, that's ``2**31 - 1``. Mandatory: there is no portable
            type-extreme derivable from a value alone, and giving callers an
            implicit one would silently bake in a backend assumption.
...
def device_reduce_max(input, identity, *, out):  # pylint: disable=redefined-builtin
    """Compute ``out[0] = max(input)`` on the device. Mirror of
    :func:`device_reduce_min` with ``max`` and the dtype's *negative* extremum
    (``-inf`` for ``f32``, ``-2**31`` for ``i32``)."""
