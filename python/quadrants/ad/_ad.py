...
"""Quadrants automatic differentiation module.
...
import quadrants.types.primitive_types as types
from quadrants import _snode
...
from quadrants.types import ndarray, template
from quadrants.types.enums import AutodiffMode, SNodeGradType
...
class Tape:
    def __init__(self, loss=None, clear_gradients=True, validation=False, grad_check=None):
        """A context manager for reverse mode autodiff :class:`~quadrants.ad.Tape`. The
        context manager would catching all of the callings of functions that
        decorated by :func:`~quadrants.lang.kernel_impl.kernel` or
        :func:`~quadrants.ad.grad_replaced` under `with` statement, and calculate
        ...
        See also :func:`~quadrants.lang.kernel_impl.kernel` and
        :func:`~quadrants.ad.grad_replaced` for gradient functions.

        Args:
            loss(:class:`~quadrants.lang.expr.Expr`): The loss field, which shape should be ().
...
def clear_all_gradients(gradient_type=SNodeGradType.ADJOINT):
    ...
    def visit(node):
        ...
        places = tuple(places)
        if places:
            from quadrants._kernels import clear_gradients  # pylint: disable=C0415
...
def grad_for(primal):
    ...
    See :func:`~quadrants.lang.grad_replaced` for examples.

    Args:
        primal (Callable): The primal function, must be decorated by :func:`~quadrants.ad.grad_replaced`.
