...
def graph_do_while(condition) -> bool:
    ...
    repeat. Work placed before or after the block at the kernel top level --
    a ``for``-loop or a bare statement -- runs exactly once, so one-time init
    / writeback can live in the same kernel (or in separate non-graph
    kernels). Loop-carried state held in global memory carries normally
    between iterations because nothing outside the loop body resets it.

    .. warning::
        Reset the condition flag **outside** the loop body, never inside it.
        A reset such as ``counter[()] = N`` placed within the ``while`` block
        is re-applied every iteration and the loop never terminates. Reset it
        before the loop (a bare top-level statement runs once) or on the host
        between launches (``counter.fill(N)``). See
        ``docs/source/user_guide/graph.md`` for the loop-carried-state idiom.

    Bare statements (assignments, ``if``, ``@qd.func`` calls) are allowed at
    the kernel top level, inside a ``graph_do_while`` body, and inside a
    ``qd.checkpoint()`` body; each runs at the loop level it is written at
    (top level = once, inside a loop = every iteration). The one structural
    rule is that a ``qd.graph_do_while`` ``while``-loop may appear only at the
    kernel top level or directly inside another ``graph_do_while`` body, not
    inside a ``for``-loop.
