...
def graph_do_while(condition) -> bool:
    ...
    ``flag`` (a scalar ``qd.i32`` ndarray) is non-zero. ``flag`` may be a bare kernel parameter (``counter``) or a
    ``@qd.data_oriented`` member ndarray accessed through ``self`` (``self.counter``).
