...
def checkpoint(*, yield_on=None):
    ...
    If ``yield_on`` is a scalar ``qd.i32`` ndarray that is also a kernel parameter, an auto-inserted yield-check kernel
    runs after the checkpoint body. If the user wrote a non-zero value into ``yield_on`` during the body, the framework
    records this checkpoint as the yielding one, disables downstream checkpoints in the enclosing ``qd.graph_do_while``
    body, exits the loop, and reports the ``cp_id`` to the host via the ``GraphStatus`` returned by the kernel call (see
    ``graph.md``). The yield-check kernel resets ``yield_on`` back to 0 after reading it; the user does not have to
    clear it from the host.

    Restrictions (enforced at kernel compile time):
      - Must be used inside ``@qd.kernel(graph=True)``.
      - ``yield_on``, when supplied, must name a kernel parameter that is a 0-d ``qd.types.ndarray(qd.i32, ndim=0)``.
