...
def _check_no_unpacked_collision(name, members):
    """Reject a member name that collides with a synthetic field generated by an earlier ``UnpackedVector`` group in the
    same struct. Called from ``StructType.__init__`` for each non-``UnpackedVector`` member; the in-loop branch handles
    the reverse case (synthetic field colliding with an already-declared member)."""
    if name in members:
        raise QuadrantsSyntaxError(
            f"member name {name!r} collides with a synthetic field generated by an UnpackedVector group "
            f"earlier in this struct. Rename the colliding field or the group."
        )


def expand_unpacked_vector_into(group_name, marker, members, unpacked_groups, elements, cook_dtype):
    """Expand an ``UnpackedVector[dtype, count]`` marker into ``count`` synthetic scalar members.

    Mutates ``members``, ``unpacked_groups``, and ``elements`` in place. Called from ``StructType.__init__``; kept as a
    free function here (rather than a method on ``StructType``) so the ``unpacked_vector`` feature surface lives in one
    module. ``cook_dtype`` is injected to avoid an import cycle (``struct.py`` -> ``unpacked_vector.py`` ->
    ``lang.util``).
    """
    cooked = cook_dtype(marker.dtype)
    unpacked_groups[group_name] = (marker.count, cooked, _expand_unpacked_vector_naming)
    for i in range(marker.count):
        sub = _expand_unpacked_vector_naming(group_name, i)
        ...
        if sub in members:
            raise QuadrantsSyntaxError(
                f"UnpackedVector group {group_name!r} would expand to a synthetic field {sub!r} that collides "
                f"with another member of the same struct. Rename the colliding field or the group."
            )
        members[sub] = cooked
...
def attach_unpacked_groups(target, unpacked_groups):
    """Tag a ``Struct`` expression-object or ``StructField`` with an ``UnpackedVector`` group dictionary so the AST
    transformer can rewrite ``obj.{group}[i]`` (or ``f[i].{group}[k]``) into direct synthetic-field references.
