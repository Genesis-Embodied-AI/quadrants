...
def writeback_binary(foo):
    ...
    def wrapped(a, b):
        ...
        if not (is_quadrants_expr(a) and a.ptr.is_lvalue()):
            raise QuadrantsSyntaxError(f"cannot use a non-writable target as the first operand of '{foo.__name__}'")
...
def cast(obj, dtype):
    ...
    Args:
        obj (Union[:mod:`~quadrants.types.primitive_types`, :class:`~quadrants.Matrix`]): \
            Input scalar or matrix.

        dtype (:mod:`~quadrants.types.primitive_types`): A primitive type defined in :mod:`~quadrants.types.primitive_types`.
...
def bit_cast(obj, dtype):
    ...
    Args:
        obj (:mod:`~quadrants.types.primitive_types`): Input scalar.

        dtype (:mod:`~quadrants.types.primitive_types`): Target data type, must have \
...
def _binary_operation(quadrants_op, python_op, a, b):
    if isinstance(a, Field) or isinstance(b, Field):
        return NotImplemented
...
def _ternary_operation(quadrants_op, python_op, a, b, c):
...
def cos(x):
    ...
    Args:
        x (Union[:mod:`~quadrants.type.primitive_types`, :class:`~quadrants.Matrix`]): \
            Angle, in radians.
...
def asin(x):
    ...
    For input `x` not in the domain `[-1, 1]`, this function returns `nan` if \
        it's called in quadrants scope, or raises exception if it's called in python scope.
...
def round(x, dtype=None):  # pylint: disable=redefined-builtin
    ...
    Args:
        ...
        dtype: (:mod:`~quadrants.types.primitive_types`): the returned type, default to `None`. If \
...
def random(dtype=float) -> Union[float, int]:
    ...
    Args:
        dtype (:mod:`~quadrants.types.primitive_types`): Type of the required random value.
...
def mod(x1, x2):
    ...
    Args:
        x1 (Union[:mod:`~quadrants.types.primitive_types`, :class:`~quadrants.Matrix`]): \
            Dividend scalar or matrix.

        x2 (Union[:mod:`~quadrants.types.primitive_types`, :class:`~quadrants.Matrix`]): \
...
def pow(base, exponent):  # pylint: disable=W0622
    ...
    Args:
        base (Union[:mod:`~quadrants.types.primitive_types`, :class:`~quadrants.Matrix`]): \
            The bases.
        exponent (Union[:mod:`~quadrants.types.primitive_types`, :class:`~quadrants.Matrix`]): \
            The exponents.
...
def raw_div(x1, x2):
    ...
    Args:
        x1 (Union[:mod:`~quadrants.types.primitive_types`, :class:`~quadrants.Matrix`]): Dividend.
        x2 (Union[:mod:`~quadrants.types.primitive_types`, :class:`~quadrants.Matrix`]): Divisor.
...
def select(cond, x1, x2):
    ...
    Args:
        cond (Union[:mod:`~quadrants.types.primitive_types`, :class:`~quadrants.Matrix`]): \
            The array of conditions.
        x1, x2 (Union[:mod:`~quadrants.types.primitive_types`, :class:`~quadrants.Matrix`]): \
...
def ifte(cond, x1, x2):
    ...
    Args:
        cond (:mod:`~quadrants.types.primitive_types`): \
            The condition.
        x1, x2 (:mod:`~quadrants.types.primitive_types`): \
            The outputs.
...
def max(*args):  # pylint: disable=W0622
    ...
    Args:
        args: (List[:mod:`~quadrants.types.primitive_types`, :class:`~quadrants.Matrix`]): \
            The input.
