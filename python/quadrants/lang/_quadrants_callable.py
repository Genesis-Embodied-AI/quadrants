...
class QuadrantsCallable:
    ...
    def __set_name__(self, owner: type, name: str) -> None:
        # Captured at class-body time. ``data_oriented.make_kernel_indirect`` sets this
        # explicitly on its replacement callable since setattr-after-class doesn't trigger
        # __set_name__.
    ...
    def __get__(self, instance, owner):
        ...
        # Non-data descriptor (no __set__): a __dict__ entry on the instance wins over the
        # descriptor on subsequent attribute lookups. Stash the bound callable there so future
        # ``instance.method`` accesses skip __get__ allocation entirely (~0.6-1.2 us/call).
        # Skip if the class uses __slots__ (no __dict__) or the attribute name wasn't captured.
