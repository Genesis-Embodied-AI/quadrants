...
def serialize(ndarray: Any) -> dict[str, Any]:
    ...
    # Use the canonical shape so it matches the canonical view returned
    # by ``to_numpy()``. Layout tags are intentionally dropped on pickle
    # (a separate ``_qd_layout`` round-trip is tracked as a follow-up):
    # the restored ndarray is always layout-free, but its element values
    # at every canonical index match the original.
    return {
