...
def mem_fence():
    ...
    if arch_uses_spv(arch):
        # On Vulkan: `OpMemoryBarrier(ScopeDevice, ...)`. On Metal (via MoltenVK + SPIRV-Cross
        # -> MSL) this is translated to `atomic_thread_fence(metal::memory_scope_device)` on
        # Apple Silicon / macOS 10.13+. Cross-workgroup ordering guarantees on older Apple
        # hardware and very old macOS Intel GPUs are weaker than on CUDA -- see `block.md`
        # for the support note.
