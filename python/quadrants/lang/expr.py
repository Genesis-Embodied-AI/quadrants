...
if TYPE_CHECKING:
    from quadrants.lang.ast.ast_transformer_utils import ASTBuilder


# Scalar, basic data type
class Expr(QuadrantsOperations):
    ...
    def get_shape(self):
        if not self.is_tensor():
            raise QuadrantsCompilationError(f"Getting shape of non-tensor type: {self.ptr.get_rvalue_type()}")
    ...
    def n(self):
        ...
        if len(shape) < 1:
            raise QuadrantsCompilationError(f"Getting n of tensor type < 1D: {self.ptr.get_rvalue_type()}")
        return shape[0]
    ...
    def m(self):
        shape = self.get_shape()
        if len(shape) < 2:
            raise QuadrantsCompilationError(f"Getting m of tensor type < 2D: {self.ptr.get_rvalue_type()}")
        return shape[1]
...
def make_constant_expr(val, dtype):
    ...
    raise QuadrantsTypeError(f"Invalid constant scalar data type: {type(val)}")
...
def make_expr_group(*exprs):
    from quadrants.lang.matrix import Matrix  # pylint: disable=C0415
