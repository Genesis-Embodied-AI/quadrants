        ...
        for k, dtype in kwargs.items():
            if k == "__struct_methods":
                ...
                for i in range(dtype.count):
                    ...
                    # Reject collisions with previously-declared fields so the user gets a clear error rather than a
                    # silent overwrite that would route ``s.r[i]`` to the wrong storage. Note this only catches
                    # collisions with members already processed in this loop -- if the colliding field is declared
                    # *after* the UnpackedVector group, the second-pass branch below catches it.
                    if sub in self.members:
                        raise QuadrantsSyntaxError(
                            f"UnpackedVector group {k!r} would expand to a synthetic field {sub!r} that collides "
                ...
                if k in self.members:
                    raise QuadrantsSyntaxError(
                        f"member name {k!r} collides with a synthetic field generated by an UnpackedVector group "
