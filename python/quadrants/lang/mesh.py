...
from quadrants.lang.matrix import Matrix, MatrixField
from quadrants.lang.struct import StructField
from quadrants.lang.util import python_scope
from quadrants.types import u16, u32
        ...
        for key, dtype in members.items():
            if key in {"verts", "edges", "faces", "cells"}:
                raise QuadrantsSyntaxError(
                    f"'{key}' cannot use as attribute name. It has been reserved as MeshQuadrants's keyword."
                )
            if key in self.attr_dict:
                raise QuadrantsSyntaxError(f"'{key}' has already use as attribute name.")
...
class MeshBuilder:
    def __init__(self):
        if not lang.misc.is_extension_supported(impl.current_cfg().arch, lang.extension.mesh):
            raise Exception("Backend " + str(impl.current_cfg().arch) + " doesn't support MeshQuadrants extension")
    ...
    def __init__(self, mesh: MeshInstance, element_type: MeshElementType, entry_expr: impl.Expr):
        ...
        for key, attr in element_field.field_dict.items():
            ...
            if isinstance(attr, MatrixField):
                ...
                raise RuntimeError("MeshQuadrants has not support StructField yet")
