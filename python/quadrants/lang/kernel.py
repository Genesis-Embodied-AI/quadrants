...
class ASTGenerator:
    ...
    def __call__(self, kernel_cxx: KernelCxx):
        ...
        try:
            ...
            # was already rebuilt from the cached (cond_arg_name, parent_id, cond_cpp_arg_id) triples in
            # _try_load_fastcache.
        ...
        if self.runtime.src_ll_cache and self.quadrants_callable and self.quadrants_callable.is_pure:
            ...
            if used_py_dataclass_parameters is not None and frontend_cache_key is not None:
                ...
                if self.compiled_kernel_data_by_key[key]:
                    ...
                    # the legacy outermost-arg alias) from the cached (cond_arg_name, parent_id,
                    # cond_cpp_arg_id) triples -- the arg-id is restored directly since there is no AST
                    # pass to re-resolve it.
