        ...
        if self.runtime.src_ll_cache and self.quadrants_callable and self.quadrants_callable.is_pure:
            ...
            if cache_value is not None:
                ...
                if self.compiled_kernel_data_by_key[key]:
                    ...
                    # `checkpoint_transformer.py` + `build_While` would have written. Prefer L2's cached levels; fall
                    # back to L1's if L2 didn't carry them (or schema mismatch).
                    _cached_levels = cache_value.graph_do_while_levels or cached_graph_do_while_levels
                    if _cached_levels:
            ...
            # table from L1's cached levels so the cold-compile path can refer to it before the AST transformer
            # repopulates it.
