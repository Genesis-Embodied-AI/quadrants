...
AST-build-time attribute access run.
...
Genesis uses two different struct shapes depending on the global
``gs.use_ndarray`` flag (see ``genesis/utils/array_class.py``):

- ``gs.use_ndarray=True``:  ``@dataclass(frozen=True)`` struct, kernel
  takes the struct as a single arg and the per-field annotation
  ``qd.types.NDArray[...]`` makes Quadrants recurse and bind each
  field as a separate kernel arg. Tests that mirror this shape are
  parametrised over the **ndarray** backend only.
- ``gs.use_ndarray=False``: ``@qd.data_oriented`` struct, kernel takes
