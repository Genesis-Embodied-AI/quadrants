...
from quadrants.lang.simt._tile32 import _TILE, _make_tile32x32
...
def test_tile32_zeros(tensor_type, qd_dtype, use_zeros_alias):
    test_utils.skip_if_f64_unsupported(qd_dtype)
    np_dtype = _NP_DTYPES[qd_dtype]
    Tile = _make_tile32x32(qd_dtype)
    ...
    def k1(dst_arr: Ann):
        qd.loop_config(block_dim=qd.simt.Tile32x32.SIZE)
        tile_size = qd.simt.Tile32x32.SIZE
...
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_eye(tensor_type, qd_dtype, inplace):
    test_utils.skip_if_f64_unsupported(qd_dtype)
    np_dtype = _NP_DTYPES[qd_dtype]
    Tile = _make_tile32x32(qd_dtype)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (0, 0, 0, 0, _TILE, 10),
        (8, 4, 2, 6, 7, 12),
        (60, 60, 0, 0, _TILE, _TILE),
    ],
)
@pytest.mark.parametrize("tensor_type", [qd.ndarray, qd.field])
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_load_store(tensor_type, qd_dtype, src_row, src_col, row_offset, col_offset, ncols, nrows):
    test_utils.skip_if_f64_unsupported(qd_dtype)
    np_dtype = _NP_DTYPES[qd_dtype]
    GRID = 92
    Tile = _make_tile32x32(qd_dtype)
...
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_clamp_to_array_rows(tensor_type, qd_dtype, clamp_side):
...
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_3d_clamp_to_array_rows(tensor_type, qd_dtype, clamp_side):
...
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_load3d_store3d(tensor_type, qd_dtype, batch, src_row, src_col, ncols, nrows):
    test_utils.skip_if_f64_unsupported(qd_dtype)
    np_dtype = _NP_DTYPES[qd_dtype]
    GRID = 92
    NBATCH = 6
...
def test_tile32_size_constant():
    Tile = _make_tile32x32(qd.f32)
    assert Tile.SIZE == 32


@test_utils.test(arch=qd.gpu)
def test_tile32_size_constant_in_kernel():
    ...
    k1(out)
    assert out.to_numpy()[0] == 32


@pytest.mark.parametrize("tensor_type", [qd.ndarray, qd.field])
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_load_clamp_to_array_cols(tensor_type, qd_dtype):
    """Load from an array narrower than 32 columns. Columns beyond arr width should be zero."""
    test_utils.skip_if_f64_unsupported(qd_dtype)
    np_dtype = _NP_DTYPES[qd_dtype]
    NCOLS = 10
    Tile = _make_tile32x32(qd_dtype)
...
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_store_partial_cols_untouched(tensor_type, qd_dtype):
    """Load full 32 columns, store only NCOLS. Remaining dst columns must be untouched."""
    test_utils.skip_if_f64_unsupported(qd_dtype)
    np_dtype = _NP_DTYPES[qd_dtype]
    NCOLS = 10
    Tile = _make_tile32x32(qd_dtype)
...
def test_tile32_make_caching():
    """_make_tile32x32 must return the same object for the same dtype."""
    a = _make_tile32x32(qd.f32)
    b = _make_tile32x32(qd.f32)
    assert a is b
    c = _make_tile32x32(qd.f64)
    assert a is not c
    d = _make_tile32x32(qd.f64)
    assert c is d
...
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_ger_sub(tensor_type, qd_dtype):
    test_utils.skip_if_f64_unsupported(qd_dtype)
    np_dtype = _NP_DTYPES[qd_dtype]
    Tile = _make_tile32x32(qd_dtype)
...
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_cholesky(tensor_type, qd_dtype, src_offset, dst_delta):
    test_utils.skip_if_f64_unsupported(qd_dtype)
    np_dtype = _NP_DTYPES[qd_dtype]
    GRID = 128
    Tile = _make_tile32x32(qd_dtype)
...
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_trsm(tensor_type, qd_dtype):
    test_utils.skip_if_f64_unsupported(qd_dtype)
    np_dtype = _NP_DTYPES[qd_dtype]
    Tile = _make_tile32x32(qd_dtype)
...
@test_utils.test(arch=qd.gpu)
def test_tile32_solve_triangular_upper_raises():
    Tile = _make_tile32x32(qd.f32)
...
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_slice_load_store_roundtrip(tensor_type, qd_dtype):
    test_utils.skip_if_f64_unsupported(qd_dtype)
    np_dtype = _NP_DTYPES[qd_dtype]
    Tile = _make_tile32x32(qd_dtype)
    src = tensor_type(qd_dtype, (_TILE, _TILE))
...
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_slice_partial_cols(tensor_type, qd_dtype):
    test_utils.skip_if_f64_unsupported(qd_dtype)
    np_dtype = _NP_DTYPES[qd_dtype]
    Tile = _make_tile32x32(qd_dtype)
    NCOLS = 7
...
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_slice_3d_batch(tensor_type, qd_dtype):
    test_utils.skip_if_f64_unsupported(qd_dtype)
    np_dtype = _NP_DTYPES[qd_dtype]
    Tile = _make_tile32x32(qd_dtype)
    NBATCH = 3
...
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_slice_ger_sub_via_outer(tensor_type, qd_dtype):
    test_utils.skip_if_f64_unsupported(qd_dtype)
    np_dtype = _NP_DTYPES[qd_dtype]
    Tile = _make_tile32x32(qd_dtype)
    mat = tensor_type(qd_dtype, (_TILE, _TILE))
...
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_vec_proxy_ger_sub_2d(qd_dtype):
    test_utils.skip_if_f64_unsupported(qd_dtype)
    np_dtype = _NP_DTYPES[qd_dtype]
    Tile = _make_tile32x32(qd_dtype)
...
@test_utils.test(arch=qd.gpu)
def test_tile32_outer_symmetric_same_variable():
...
@test_utils.test(arch=qd.gpu)
def test_tile32_vec_proxy_ger_sub_3d():
...
@test_utils.test(arch=qd.gpu)
def test_tile32_load_slice_errors(bad_slice, match):
    Tile = _make_tile32x32(qd.f32)
...
@test_utils.test(arch=qd.gpu)
def test_tile32_store_slice_errors(bad_slice, match):
    Tile = _make_tile32x32(qd.f32)
    src = qd.ndarray(qd.f32, (_TILE, _TILE))
    dst = qd.ndarray(qd.f32, (_TILE, _TILE))
...
@test_utils.test(arch=qd.gpu)
def test_tile32_slice_wrong_index_order_raises():
...
@test_utils.test(arch=qd.gpu)
def test_tile32_slice_extra_indices_raises():
...
@test_utils.test(arch=qd.gpu)
def test_tile32_outer_product_intermediate_variable():
...
@test_utils.test(arch=qd.gpu)
def test_tile32_load_without_slice_rebinds():
...
@test_utils.test(arch=qd.gpu)
def test_tile32_augassign_add_outer_raises():
...
@test_utils.test(arch=qd.gpu)
def test_tile32_augassign_non_outer_raises():
...
@test_utils.test(arch=qd.gpu)
def test_tile32_vec_slice_errors(bad_slice):
    Tile = _make_tile32x32(qd.f32)
...
# =============================================================================
# PR 4: public-API tests (qd.simt.Tile32x32 proxy, tensor_type, SharedArray)
# =============================================================================
...
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_load_store_partial(tensor_type, qd_dtype):
    test_utils.skip_if_f64_unsupported(qd_dtype)
    np_dtype = _NP_DTYPES[qd_dtype]
    NCOLS = 12
    src = tensor_type(qd_dtype, (_TILE, _TILE))
    ...
    def k1(src_arr: Ann, dst_arr: Ann, NCOLS: qd.i32):
        ...
        tile_size = qd.simt.Tile32x32.SIZE
        for _ in range(tile_size):
            t = qd.simt.Tile32x32.zeros(dtype=qd_dtype)
            t[:] = src_arr[0:tile_size, 0:NCOLS]
            dst_arr[0:tile_size, 0:tile_size] = t
...
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_load_store_partial_rows(qd_dtype, nrows):
    """Load/store with fewer than 32 rows -- threads beyond nrows should be skipped."""
    test_utils.skip_if_f64_unsupported(qd_dtype)
    np_dtype = _NP_DTYPES[qd_dtype]
    GRID = 48
...
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_syr_sub(tensor_type, qd_dtype):
    test_utils.skip_if_f64_unsupported(qd_dtype)
    np_dtype = _NP_DTYPES[qd_dtype]
    mat = tensor_type(qd_dtype, (_TILE, _TILE))
...
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_potrf(tensor_type, qd_dtype):
    test_utils.skip_if_f64_unsupported(qd_dtype)
    np_dtype = _NP_DTYPES[qd_dtype]
...
@pytest.mark.parametrize("qd_dtype", _QD_DTYPES)
@test_utils.test(arch=qd.gpu)
def test_tile32_potrf_then_trsm(tensor_type, qd_dtype):
    test_utils.skip_if_f64_unsupported(qd_dtype)
    np_dtype = _NP_DTYPES[qd_dtype]
    atol = _ATOLS[qd_dtype]
    ...
    def k1(a_arr: Ann, b_arr: Ann, x_arr: Ann, eps_f: qd.Template):
        ...
        tile_size = qd.simt.Tile32x32.SIZE
        for _ in range(tile_size):
            L = qd.simt.Tile32x32.zeros(dtype=qd_dtype)
            L[:] = a_arr[0:tile_size, 0:tile_size]
            L.cholesky_(eps_f[None])
            B = qd.simt.Tile32x32.zeros(dtype=qd_dtype)
...
@test_utils.test(arch=qd.gpu)
def test_tile32_shared_array_roundtrip():
    ...
    def k1(src_f: qd.Template, dst_f: qd.Template):
        ...
        tile_size = qd.simt.Tile32x32.SIZE
        for _ in range(tile_size):
            sh = qd.simt.block.SharedArray((qd.simt.Tile32x32.SIZE, qd.simt.Tile32x32.SIZE), qd.f32)
            t = qd.simt.Tile32x32.zeros(dtype=qd.f32)
            t[:] = src_f[0:tile_size, 0:tile_size]
            sh[0:tile_size, 0:tile_size] = t
            qd.simt.block.sync()
            t2 = qd.simt.Tile32x32.zeros(dtype=qd.f32)
...
@test_utils.test(arch=qd.gpu)
def test_tile32_shared_array_partial_cols(partial_store, partial_load):
...
@test_utils.test(arch=qd.gpu)
def test_tile32_shared_array_cholesky():
...
@test_utils.test(arch=qd.gpu)
def test_tile32_shared_array_clamp_store():
    """Store tile to SharedArray narrower than 32 cols. Must auto-clamp, no OOB."""
    NCOLS = 10
    src = qd.field(dtype=qd.f32, shape=(_TILE, _TILE))
    ...
    def k1(src_f: qd.Template, dst_f: qd.Template, NCOLS: qd.i32):
        ...
        tile_size = qd.simt.Tile32x32.SIZE
        for _ in range(tile_size):
            sh = qd.simt.block.SharedArray((qd.simt.Tile32x32.SIZE, 10), qd.f32)
            t = qd.simt.Tile32x32.zeros(dtype=qd.f32)
            t[:] = src_f[0:tile_size, 0:tile_size]
            sh[0:tile_size, 0:tile_size] = t
            qd.simt.block.sync()
...
@test_utils.test(arch=qd.gpu)
def test_tile32_shared_array_clamp_load():
    """Load tile from SharedArray narrower than 32 cols. Must auto-clamp, extra regs zero."""
    NCOLS = 10
    ...
    def k1(src_f: qd.Template, dst_f: qd.Template, NCOLS: qd.i32):
        ...
        for _ in range(tile_size):
            sh = qd.simt.block.SharedArray((qd.simt.Tile32x32.SIZE, 10), qd.f32)
            t_load = qd.simt.Tile32x32.zeros(dtype=qd.f32)
...
@pytest.mark.parametrize("tensor_type", [qd.ndarray, qd.field])
@test_utils.test(arch=qd.gpu)
def test_tile32_vec_proxy_syr_sub_2d(tensor_type):
    ...
    def k1(mat_arr: Ann_tile, vecs_arr: Ann_vecs, out_arr: Ann_tile, K0: qd.i32, COL: qd.i32):
        ...
        for _ in range(tile_size):
            t = qd.simt.Tile32x32.zeros(dtype=qd.f32)
            t[:] = mat_arr[0:tile_size, 0:tile_size]
            v = vecs_arr[K0 : K0 + qd.simt.Tile32x32.SIZE, COL]
            t -= qd.outer(v, v)
            out_arr[0:tile_size, 0:tile_size] = t
    ...
    col = V[K0 : K0 + _TILE, COL]
    np.testing.assert_allclose(out.to_numpy(), R - np.outer(col, col), atol=1e-5)


@pytest.mark.parametrize("tensor_type", [qd.ndarray, qd.field])
@test_utils.test(arch=qd.gpu)
def test_tile32_vec_proxy_syr_sub_3d(tensor_type):
    ...
    def k1(mat_arr: Ann_tile, vecs_arr: Ann_vecs, out_arr: Ann_tile, K0: qd.i32, COL: qd.i32):
        ...
        for _ in range(tile_size):
            t = qd.simt.Tile32x32.zeros(dtype=qd.f32)
            t[:] = mat_arr[0:tile_size, 0:tile_size]
            v = vecs_arr[1, K0 : K0 + qd.simt.Tile32x32.SIZE, COL]
            t -= qd.outer(v, v)
            out_arr[0:tile_size, 0:tile_size] = t
    ...
    vecs.from_numpy(V)
    k1(mat, vecs, out, K0, COL)
    col = V[1, K0 : K0 + _TILE, COL]
    np.testing.assert_allclose(out.to_numpy(), R - np.outer(col, col), atol=1e-5)


@test_utils.test(arch=qd.gpu)
def test_tile32_vec_proxy_shared_array():
    ...
    def k1(mat_f: qd.Template, vecs_f: qd.Template, out_f: qd.Template, K0: qd.i32, COL: qd.i32, m_size: qd.i32):
        ...
        for _ in range(tile_size):
            ...
            v = sh[K0 : K0 + qd.simt.Tile32x32.SIZE, COL]
            t -= qd.outer(v, v)
...
@pytest.mark.parametrize("tensor_type", [qd.ndarray, qd.field])
@test_utils.test(arch=qd.gpu)
def test_tile32_vec_proxy_partial_rows(tensor_type):
    """Vec proxy with partial last block: only _M-K0 of _TILE threads contribute."""
    mat = tensor_type(qd.f32, (_TILE, _TILE))
    vecs = tensor_type(qd.f32, (_M, _M))
    out = tensor_type(qd.f32, (_TILE, _TILE))
...
@pytest.mark.parametrize("tensor_type", [qd.ndarray, qd.field])
@test_utils.test(arch=qd.gpu)
def test_tile32_vec_proxy_multi_column_accumulate(tensor_type):
    ...
    def k1(mat_arr: Ann_tile, vecs_arr: Ann_vecs, out_arr: Ann_tile, K0: qd.i32, NCOLS: qd.i32):
        ...
        for _ in range(tile_size):
            ...
            for c in range(NCOLS):
                v = vecs_arr[K0 : K0 + qd.simt.Tile32x32.SIZE, c]
                t -= qd.outer(v, v)
            out_arr[0:tile_size, 0:tile_size] = t
...
def test_tile32_f64_roundtrip_into_f32_array():
    ...
    def roundtrip_f64(s: Ann, d: Ann):
        ...
        tile_size = qd.simt.Tile32x32.SIZE
        for _ in range(tile_size):
            t = qd.simt.Tile32x32.zeros(dtype=qd.f64)
            t[:] = s[0:tile_size, 0:tile_size]
            d[0:tile_size, 0:tile_size] = t
...
def test_tile32_raises_on_cpu():
    """Using Tile32x32 on a CPU backend must raise QuadrantsSyntaxError, not crash."""
...
# -- Proxy tests (qd.simt.Tile32x32 as a proxy object) --


@test_utils.test(arch=qd.gpu)
def test_proxy_size_constant():
    assert qd.simt.Tile32x32.SIZE == 32
...
def test_proxy_eye_explicit_dtype(qd_dtype):
    ...
    def k1(d: qd.types.NDArray[qd_dtype, 2]):
        ...
        tile_size = qd.simt.Tile32x32.SIZE
        for _ in range(tile_size):
            t = qd.simt.Tile32x32.eye(dtype=qd_dtype)
            d[0:tile_size, 0:tile_size] = t
...
def test_tile32_cholesky_blocked_demo():
