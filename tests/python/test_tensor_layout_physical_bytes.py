...
The ``layout=`` kwarg on ``qd.tensor`` is documented as a *physical
memory permutation*: ``shape`` is the canonical (user-indexed) shape,
and ``layout`` describes the order in which canonical axes are
materialised in memory, outermost first. The promise to downstream
users (in particular Genesis kernels) is that:
...
allocate **byte-identical** physical buffers. Equivalently: indexing
the second tensor with a permuted canonical key writes to the *same*
raw memory address as indexing the first tensor with the natural key.

That promise drives the canonical-shape rewrites Genesis is doing for
its constraint-state tensors, where shape ``(n_constraints, B)`` is
being rotated to ``(B, n_constraints)`` with ``layout=(1, 0)`` so that
batch-major access patterns at the call sites read more naturally
without changing the underlying memory order. If the rewrite quietly
produced a different physical buffer, every kernel access would land
on a different cache line and the rewrite would silently regress
performance.

Existing layout tests in ``test_tensor_layout.py`` /
``test_tensor_factory_layout_ndarray.py`` /
``test_tensor_layout_interop.py`` cover the canonical-view contract
(``shape``, ``to_numpy``, ``to_torch``, ``to_dlpack``, ``.grad``), and
``test_to_dlpack_layout_shows_up_as_strides_not_shape`` confirms the
DLPack view uses non-contiguous strides on layout-tagged tensors. None
of them assert that the *raw bytes* of two equivalent (canonical,
layout) configurations are bit-identical, which is the property this
file pins.

Method: write a single sentinel via host setitem at canonical-equivalent
positions on both tensors, dump the raw 1-D byte view via DLPack
(which, unlike ``to_torch()``, returns a faithful non-copy view of the
underlying allocation), and assert the raw arrays compare equal.
"""
...
def test_layout_rank2_transpose_is_byte_identical_to_default(backend):
    """``qd.tensor(shape=(N, B))`` and
    ``qd.tensor(shape=(B, N), layout=(1, 0))`` must have byte-identical
...
def test_layout_rank2_transpose_full_pattern_byte_identical(backend):
    """Same as the sentinel test, but writes a unique value at every
    canonical position so the assertion catches any single-element
    permutation bug, not just one corner."""
    pytest.importorskip("torch")
    qd.init(arch=qd.x64)
    N, B = 5, 7
...
def test_layout_rank2_kernel_writes_byte_identical(backend):
    """Same canonical fill kernel run on parent and rotated tensors must
    produce byte-identical physical buffers."""
    pytest.importorskip("torch")
    qd.init(arch=qd.x64)
    N, B = 5, 7
...
# ---------------------------------------------------------------------------
# Negative control on NDARRAY (whose physical allocation is not subject to
# the field-side global SNode root-tree fusion that can collapse
# distinct-shape ``qd.field`` allocations into surprising layouts on the
# CPU x64 backend). Without ``layout=(1, 0)``, simply swapping the
# canonical shape must produce a *different* physical buffer.
# ---------------------------------------------------------------------------
...
def test_layout_negative_control_ndarray_swapped_shape_no_layout_differs():
    ...
    # The two buffers must NOT match: without ``layout=(1, 0)``, shape
    # ``(B, N)`` row-major lays ``b`` out as the slow axis, not ``c``.
