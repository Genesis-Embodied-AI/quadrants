...
def test_graph_parallel_region_id_not_aliased_by_per_task_cache():
    """The per-task codegen cache must not merge two region groupings.

    The per-Program per-task cache keys a compiled task on its printed IR (+ config/caps/SNode ABI/
    autodiff), and the IR printer does not emit ``graph_parallel_region_id`` / ``stream_parallel_group_id``.
    So two graph kernels in one Program whose tasks are byte-identical at the same index but grouped into
    different fork/join regions can collide: the second kernel warm-hits the first's cached modules and
    inherits the first's region ids. Here ``k_two_regions`` would then see all four sections in one region,
    drop the inter-region join, and let region B fork alongside (and race) region A. Regression test for the
    per-task key omitting the graph-region tag; guarded on CUDA where the fork/join is built.
    """
    n = 256

    # Four sections in ONE region: all four section tasks share one region id.
    @qd.kernel(graph=True)
    def k_one_region(
        p: qd.types.ndarray(qd.f32, ndim=1),
        q: qd.types.ndarray(qd.f32, ndim=1),
        r: qd.types.ndarray(qd.f32, ndim=1),
        s: qd.types.ndarray(qd.f32, ndim=1),
    ):
        with qd.graph.parallel_context():
            with qd.graph.parallel():
                for i in range(n):
    ...
    # The same four section bodies, in the same order, but split into TWO back-to-back regions (2 + 2). The
    # r/s tasks are byte-identical to k_one_region's at the same index; only their region id differs.
    @qd.kernel(graph=True)
    def k_two_regions(
        p: qd.types.ndarray(qd.f32, ndim=1),
        q: qd.types.ndarray(qd.f32, ndim=1),
    ...
    def fresh():
        a = [qd.ndarray(qd.f32, shape=(n,)) for _ in range(4)]
        for arr in a:
            arr.from_numpy(np.zeros(n, dtype=np.float32))
        return a

    # Warm the per-task cache with the one-region kernel (all four tasks in region 1).
    p, q, r, s = fresh()
    k_one_region(p, q, r, s)

    # Compile the two-region kernel in the same Program. Its region-2 (r, s) tasks are byte-identical to the
    # one-region kernel's, so without the region tag in the key they would warm-hit and inherit region 1.
    p, q, r, s = fresh()
    k_two_regions(p, q, r, s)

    num_tasks = _num_offloaded_tasks()
    obs = k_two_regions._primal.per_offload_cache_observations
    # The two region-2 tasks must recompile (their region id is now part of the key), not reuse region-1
    # modules. Before the fix all four warm-hit (recompiled == 0).
    assert obs.constructs_recompiled >= 2, obs
    # The inter-region join must survive: one empty join node per region (two), not one merged join. Before
    # the fix the merged region emits only num_tasks + 1 nodes.
    assert _graph_num_nodes() == num_tasks + 2, (_graph_num_nodes(), num_tasks)

    # Sanity: each disjoint array is bumped exactly once (the kernel ran correctly).
    np.testing.assert_allclose(p.to_numpy(), 1.0)
    np.testing.assert_allclose(q.to_numpy(), 2.0)
    np.testing.assert_allclose(r.to_numpy(), 3.0)
    np.testing.assert_allclose(s.to_numpy(), 4.0)
