...
def test_io_devices():
    n = 32
    x = qd.field(dtype=qd.i32, shape=n)

    @qd.kernel
    def load(y: qd.types.ndarray()):
        for i in x:
            x[i] = y[i] + 10
...
def test_io():
    ...
    @qd.kernel
    def torch_kernel(t: qd.types.ndarray(), o: qd.types.ndarray()):
        for i in range(n):
            o[i] = t[i] * t[i]

    @qd.kernel
    def torch_kernel_2(t_grad: qd.types.ndarray(), t: qd.types.ndarray(), o_grad: qd.types.ndarray()):
        for i in range(n):
...
def test_io_simple():
    ...
    x1 = qd.field(qd.f32, shape=(n, n))
    t1 = torch.tensor(2 * np.ones((n, n), dtype=np.float32))

    x2 = qd.Matrix.field(2, 3, qd.f32, shape=(n, n))
...
@test_utils.test()
def test_io_zeros():
    mat = qd.Matrix.field(2, 6, dtype=qd.f32, shape=(), needs_grad=True)
...
def test_io_struct():
    n = 16
    x1 = qd.Struct.field({"a": qd.i32, "b": qd.f32}, shape=(n,))
    t1 = {
...
def test_torch_zero():
    @qd.kernel
    def test_torch(arr: qd.types.ndarray()):
        pass
...
def test_torch_view():
    ...
    y = qd.ndarray(int, (3, 3))

    with pytest.raises(ValueError, match=r"Non contiguous tensors are not supported"):
        copy(x, y)
