...
If any parameter (or any member of a `@qd.data_oriented` / `dataclasses.dataclass` parameter) is of an unsupported type, fastcache is disabled for that call and the kernel falls back to normal compilation. For `qd.field` / `ScalarField` / `MatrixField` arriving through a `qd.Tensor`-annotated parameter, this is silent — no warning is emitted. For other unsupported types, a `[FASTCACHE][PARAM_INVALID]` warning is logged at the `warn` level identifying the offending parameter path.
...
**`@qd.data_oriented`:** the walker descends into `vars(obj)` (or `obj._asdict()` if defined). For each child:
