...
| `qd.checkpoint` skip (IF gate) | hardware accelerated (conditional graph node) | GPU-side (codegen prologue + flat graph) | host-orchestrated sub-graphs (to be replaced) | GPU-side (gate shader + indirect dispatch) | GPU-side (gate shader + indirect dispatch) | host-branch gating |
| `qd.checkpoint(yield_on=…)` + `kernel.resume()` | implemented | implemented | implemented (host-orchestrated; to be replaced) | implemented | implemented | implemented |

`qd.checkpoint` gating happens entirely on the device for every GPU backend except the current AMDGPU path (see below). The exact device mechanism differs by backend (see "Backend coverage notes" below) but the Python contract (`qd.checkpoint`, `GraphStatus`, `kernel.resume(from_checkpoint=…)`) is identical.

AMDGPU `graph_do_while` falls back to the host-side loop because HIP does not currently expose conditional / while graph nodes (as of ROCm 7.2). HIP 7.2 also lacks an indirect-dispatch primitive, so AMDGPU `qd.checkpoint` is presently implemented via host-orchestrated sub-graph selection (split the cached HIP graph into one sub-graph per contiguous run of same-`cp_id` tasks, host decides which to launch). This violates the "all gating on the device" property the other GPU backends meet and is scheduled to be replaced with the same codegen-prologue + flat HIP graph approach used on pre-Hopper CUDA.
...
> **Status:** Implemented end-to-end on every supported backend except AMDGPU. The Python contract (`qd.checkpoint`, `GraphStatus`, `kernel.resume(from_checkpoint=…)`) is identical everywhere; only the device-side lowering differs:
>
...
> - **AMDGPU**: not yet implemented. HIP 7.2 has no conditional-node API and no indirect-dispatch primitive, so the planned approach mirrors pre-Hopper CUDA: codegen prologue + flat HIP graph. Tracked separately; today's AMDGPU path falls back to non-graph kernel launches when a kernel uses `qd.checkpoint`.
