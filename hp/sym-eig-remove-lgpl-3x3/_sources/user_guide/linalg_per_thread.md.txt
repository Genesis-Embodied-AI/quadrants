...
- **2×2 .. 12×12** — cyclic Jacobi: 12 sweeps of Givens rotations zeroing every off-diagonal `(p, q)` pair, with `Q := Q · J` accumulated as eigenvectors. ~6 digits of accuracy in `f32`, ~12 digits in `f64`.
...
- **Compile time.**
  ...
  - **Cyclic Jacobi** (`qd.sym_eig`, `qd.make_spd`) — the outer sweep loop is unrolled for `N ≤ 4` and a runtime loop for `N > 4` (unrolling it grows compile time quickly with N). Compile times for a single `qd.sym_eig` call on CUDA + LLVM 22.1 (`QD_OFFLINE_CACHE=0`): ~11 s at N=4 (unrolled); ~4 s at N=6, ~26 s at N=9, ~125 s at N=12 (runtime).
