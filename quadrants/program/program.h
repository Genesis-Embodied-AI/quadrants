...
class QD_DLL_EXPORT Program {
 public:
  ...
  // ``_full`` variants like ``reduce_add_full(v)`` can unroll the correct ``log2_size`` at compile time on every
  // backend.  Returns ``0`` on the x64 CPU backend and any backend that has not been initialized; callers that need
  // ``log2(size)`` should use ``qd.simt.subgroup.log2_group_size()`` which asserts power-of-two.
