...
class LaunchContextBuilder {
 ...
 public:
  ...
  // `qd.checkpoint(yield_on=name)`, or `-1` for checkpoints without a `yield_on=`. Set by `Kernel.__call__` just
  // before `prog.launch_kernel`, mirroring `graph_do_while_arg_id`.
