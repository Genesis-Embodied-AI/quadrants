...
class LaunchContextBuilder {
 ...
 public:
  ...
  // ndarray-resolution loop. Replaces the old single-loop scalars (arg id + flag ptr); the single
  // (non-nested) loop is simply the depth-1 case with one level whose parent_id is -1.
  ...
  // checkpoints without a `yield_on=`. Set by `Kernel.__call__` just before `prog.launch_kernel`.
