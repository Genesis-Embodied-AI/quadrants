...
static std::vector<std::uint8_t> get_offline_cache_key_of_compile_config(const CompileConfig &config) {
  ...
  if (arch_is_cpu(config.arch)) {
    ...
    // toggle. SPIR-V (Vulkan / Metal) returns ``0`` here - the device-probed
    // ``DeviceCapability::spirv_subgroup_size`` is part of ``DeviceCapabilityConfig::devcaps`` and gets mixed into the
    // key via ``get_offline_cache_key_of_device_caps`` below, so wave32 vs wave64 SPIR-V devices already get distinct
    // cache entries without needing to plumb the value through here.
