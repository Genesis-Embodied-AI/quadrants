...
#include "quadrants/inc/expressions.inc.h"
 ...
 private:
  ...
  void emit(Stmt *stmt) {
    if (stmt) {
      ...
      emit(stmt->ret_type);
      // The graph-region (graph_do_while level / checkpoint cp_id / stream_parallel group) a statement
      // sits in now affects how the offloader places it -- and graph_do_while emits no loop IR, so the
      // region is otherwise invisible to the key. Two kernels that differ only in whether a bare
      // statement is inside vs outside a graph_do_while loop must get distinct keys.
