...
Expr cast(const Expr &input) {
  return quadrants::lang::cast(input, get_data_type<T>());
}
...
// Canonical definition of `precise` semantics. The `precise` bool field on UnaryOp{Expression,Stmt},
// BinaryOp{Expression,Stmt}, and TernaryOp{Expression,Stmt} is a cross-reference to this contract.
//
...
// qd.func calls, ndarray accesses, ...). Of the TernaryOp variants, `fma` is FP arithmetic and is tagged; `select` /
// `ifte` are control-flow-shaped and left untagged. `input` is NOT mutated: walked nodes are cloned bottom-up so
// aliasing the original expression elsewhere does not retroactively inherit the tag; non-walked children are shared
// by reference since they carry no `precise` field. The tag is propagated from Expression to Stmt by each class's
// `flatten()`.
