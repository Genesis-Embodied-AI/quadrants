...
void UnaryOpExpression::flatten(FlattenContext *ctx) {
  ...
  if (precise) {
    unary->codegen_hints |= (uint32_t)CodegenHint::kDisableFastMath;
  }
  stmt = unary.get();
...
void BinaryOpExpression::flatten(FlattenContext *ctx) {
  ...
  if (precise) {
    bin_stmt->codegen_hints |= (uint32_t)CodegenHint::kDisableFastMath;
  }
...
void ASTBuilder::insert_break_stmt(const DebugInfo &dbg_info) {
  if (loop_state_stack_.back() == Outermost) {
    throw QuadrantsSyntaxError("Cannot break in the outermost loop");
  }
