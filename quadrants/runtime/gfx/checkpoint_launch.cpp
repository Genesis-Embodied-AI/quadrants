// GPU-side per-checkpoint gating for `qd.checkpoint(...)` on Vulkan / Metal. Extracted out of `runtime.cpp` so the main
// launcher's per-task dispatch loop stays focused; every code path here is conditional on at least one task carrying
// `cp_id >= 0` and never runs for vanilla kernels.
//
...
//          shader each launch, consumed via `CommandList::dispatch_indirect` by the body kernels (and the yield-check
//          shader)
//        * an optional `yield_check_params` SSBO (4 bytes, cp_id) for yielding checkpoints
//    The same pre-built gate / yield-check pipelines (one per `GfxRuntime`) are used across every yielding-capable
//    kernel - they're generic in cp_id / n_kernels / dims.
//
// 2. At launch time, `launch_kernel` calls `dispatch_checkpoint_gate` before the first body task of
//    each cp_id; the gate writes the per-kernel dim3 triples into `out_dims` based on the current `(resume_point,
//    yield_signal)` value. The body kernels then dispatch indirect off the same `out_dims`; skipped checkpoints'
//    kernels dispatch with `(0,0,0)` workgroups, which is a hardware no-op on both Vulkan (`vkCmdDispatchIndirect`) and
//    Metal (`dispatchThreadgroupsIndirect`).
//
// 3. After the last body task of a yielding checkpoint, `dispatch_checkpoint_yield_check` issues the
//    yield-check shader indirect off the trailing `out_dims` slot. The shader reads the user's `yield_on=` flag,
//    atomic-CASes the cp_id into `yield_signal` on non-zero, and resets the flag. The yield-check is itself
//    indirect-gated (same `(0,0,0)` no-op semantics as the body kernels), so a skipped checkpoint also skips its
//    yield-check.
//
// 4. After all tasks have dispatched, `launch_kernel` flushes, syncs, reads back `yield_signal` from
//    `state.control[1]`, and surfaces the first-yielder cp_id through `last_yield_cp_id_on_last_call()`.
//
...
// No per-task host gating; no per-task D2H. This replaces the slice-4 host-branch loop in the original
// `runtime.cpp::launch_kernel` that this file's helpers were built to displace.
...
// Build (and cache) the generic gate compute pipeline. Called from `ensure_checkpoint_state_for_handle`; idempotent
// across handles.
...
// Allocate (or reuse) a small device buffer of `bytes` for storage / indirect use. `host_write` / `host_read` control
// the memory-property hints; the launcher uses host_write=true on the control buffer (resume_point is set per launch),
// host_read=true on the same buffer (yield_signal is read at end), and both false on the gate params / out_dims /
// yield-check params (device-only access). `indirect` selects whether `AllocUsage::Indirect` is set in addition to
// `Storage`; required for the `out_dims` buffer so `vkCmdDispatchIndirect` can read it.
...
// Upload the host-resident `data` (u32 words) into `buf`. Used to bake the gate params buffer once at first launch and
// to write the yield-check params (cp_id) per-checkpoint at first launch.
...
bool GfxRuntime::ensure_checkpoint_state_for_handle(
  ...
  // Fast path: no task carries a checkpoint id - this kernel is not yielding-capable, no gating state to set up, the
  // launcher should use the standard direct-dispatch path.
  ...
  // First-launch setup for this handle. The per-cp layout is baked from the task list and never changes across
  // launches; subsequent launches reuse the same buffers as-is. `state.per_cp.empty()` is the "first launch" sentinel.
  if (state.per_cp.empty()) {
    ...
    for (int32_t cp = 0; cp <= max_cp_id; ++cp) {
      ...
      // Out-dims buffer is written per-launch by the gate shader and read indirectly by each body (and yield-check)
      // dispatch. Sized for `n_dispatches` u32 triples. `Indirect` usage flag is required so `vkCmdDispatchIndirect`
      // accepts the buffer.
    ...
    // Per-launch sanity: per_task_group_x must match what we baked at first launch. The active grid for a given task is
    // a function of the kernel's compile-time advisory + the host-side ndarray shape; a different shape would yield a
    // different group_x and silently mis-gate via a stale baked active dim. Quadrants currently rebuilds the kernel
    // handle whenever the advisory or shape changes (see `Kernel::launch_kernel` cache key), so this assertion exists
    ...
    for (int32_t cp = 0; cp <= max_cp_id; ++cp) {
      ...
      for (int slot = 0; slot < (int)per_cp.body_task_indices.size(); ++slot) {
        ...
        // No-op consistency check; per_task_group_x is recomputed per launch but should be stable for a given handle.
        // If this ever fires, the baked gate_params is stale and we'd need to re-upload it here (cheap; just a
        // `upload_u32_words` rewrite).
...
// Record a gate-shader dispatch into `cmdlist` for one checkpoint. The gate writes the per-kernel dim3 triples into the
// out_dims buffer based on the current `(resume_point, yield_signal)` in the control buffer. Callers must call
// `memory_barrier()` after this so subsequent body kernel indirect-dispatches see the written dims, and so the body /
// yield-check shaders' subsequent atomic updates to `yield_signal` happen-after the gate's read of it.
void GfxRuntime::dispatch_checkpoint_gate(CommandList *cmdlist, const CheckpointHandleState &state, int cp_id) {
  ...
  // Barrier the control + out_dims buffers so the gate's reads of control / writes to out_dims happen-after any prior
  // cmdlist op that touched them (e.g. an earlier yield-check shader's atomic-CAS of yield_signal, or the previous
  // task's indirect-dispatch read of out_dims).
  ...
  // Out-dims must be barriered before the subsequent body kernel indirect-dispatches read it. The barrier is left to
  // the caller (matches the per-task `memory_barrier()` that already runs after each direct dispatch); this keeps the
  // dispatch-of-things-per-task pattern consistent.
}

// Record a yield-check-shader dispatch into `cmdlist` for one yielding checkpoint, indirect-gated off the trailing slot
// of out_dims so the shader no-ops when the checkpoint was skipped. `yield_on_devalloc` is the user's `yield_on=`
// ndarray device allocation (resolved per-launch from the `host_ctx.checkpoint_yield_on_arg_ids[cp_id]` arg id lookup
// against `any_arrays`).
void GfxRuntime::dispatch_checkpoint_yield_check(CommandList *cmdlist,
  ...
  QD_ASSERT_INFO(per_cp.yield_check_params != nullptr,
                 ...
                 cp_id);
  // Barrier the user's yield_on ndarray so the yield-check's load happens-after the body kernels' writes to the flag.
  // The control buffer is barriered too because the yield-check does an atomic-CAS into yield_signal; same handle as
  // the gate's barrier above.
  ...
  QD_ERROR_IF(bind_res != RhiResult::success, "Checkpoint yield-check resource bind failed: RhiResult({})",
              int(bind_res));
  // Indirect dispatch off the trailing out_dims slot (immediately after the body kernel slots). When the gate decided
  // to skip, that slot holds (0, 0, 0) and the GPU dispatches zero workgroups - the atomic-CAS and flag reset are both
  // elided. When the gate decided to run, the slot holds (1, 1, 1) and the single-thread shader executes.
...
// Per-launch preparation of the checkpoint subsystem on the GPU side. Mirrors what was previously inlined in
// `runtime.cpp::launch_kernel` immediately after `ensure_checkpoint_state_for_handle`. Extracted here so the main
// per-task dispatch loop in `runtime.cpp` stays focused on the standard direct-dispatch path; this helper is only doing
// checkpoint-specific work and always returns an empty plan when `kernel_has_checkpoints` is false.
//
// Side effects beyond the returned plan:
//   - Conditionally resets `last_yield_cp_id_on_last_call_` to -1 (matches the CPU / AMDGPU
//     contract: a kernel that can yield clears the cross-launch status at the top of every launch; non-yielding aux
//     launches leave the prior value alone so the user's status read survives).
//   - Uploads `(resume_point, -1)` into `state.control` so the gate shader's first dispatch sees a
//     consistent starting tuple. Done before `ensure_current_cmdlist` opens the launch's cmdlist so the upload's
//     wait-for-prior-launch semantics finish before the GPU starts gating.
GfxRuntime::CheckpointLaunchPlan GfxRuntime::prepare_checkpoint_launch_state(
  ...
  // Per-cp slot map (cp_id -> dense slot index inside `state.per_cp[cp].out_dims`). Built once per launch since
  // `body_task_indices` is stable. `slot_in_cp[i] == k` means task `i` reads dim3 triple at byte offset `12 * k` of its
  // checkpoint's out_dims buffer.
  ...
  // Reset the public `last_yield_cp_id_on_last_call_` for any kernel that has at least one yielding checkpoint -
  // matches the CPU / AMDGPU semantics. Aux launches without `yield_on=` on any cp leave it alone so the user's prior
  // status read survives.
  ...
  // Resolve each cp's user-supplied `yield_on=` ndarray to a DeviceAllocation by looking up its arg id in `any_arrays`.
  // Done per-launch because the user can hand a different ndarray to the kernel each call - we don't bake the pointer
  // into anything.
  ...
  // Seed the per-launch control buffer with `(resume_point, -1)`. `upload_data` blocks on any in-flight stream work
  // touching the buffer; for the control buffer that's the previous launch's yield-check shader, which `synchronize()`
  // (called between Python-visible launches) already drained. Done before `ensure_current_cmdlist` opens this launch's
  // cmdlist so the first gate dispatch sees the freshly-uploaded values.
...
// Single end-of-launch readback of yield_signal. `synchronize()` semantics: `flush()` submits the recorded cmdlist;
// `wait_idle()` waits for the GPU; `readback_data` then maps the host-readable control buffer and reads the 8-byte
// `(resume_point, yield_signal)` tuple. We only care about `yield_signal` here - if non-`-1`, it's the cp_id of the
// first checkpoint whose `yield_on=` flag fired, propagated to the Python `GraphStatus` via
// `last_yield_cp_id_on_last_call()`.
//
// Re-opens the cmdlist on return so any downstream work (e.g. the D2H blit of return values) can keep recording; the
// `flush()` above retired the cmdlist on backends that null it out.
void GfxRuntime::finalize_checkpoint_readback(KernelHandle handle,
  ...
  // After the readback there is no `current_cmdlist_` (the flush above retired it on backends that null it out).
  // Re-open if any downstream work (the d2h blit below) needs to record into it; `ensure_current_cmdlist()` is
  // idempotent so the call is safe even on backends that keep the cmdlist alive across flush.
