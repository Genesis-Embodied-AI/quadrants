...
void GfxRuntime::launch_kernel(KernelHandle handle,
  ...
  if (profiler_) {
    ...
    if (task_count > apple_max_query_pool_count) {
      TI_WARN(
          "Cannot concurrently profile more than 32 tasks in a single Quadrants "
...
GfxRuntime::RegisterParams run_codegen(
  ...
  TI_TRACE("VK codegen for Quadrants kernel={}", quadrants_kernel_name);
