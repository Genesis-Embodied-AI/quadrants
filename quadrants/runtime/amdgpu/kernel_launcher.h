...
class KernelLauncher : public LLVM::KernelLauncher {
  ...
  struct Context {
    ...
    // GPU-side `qd.checkpoint` gating state, allocated lazily on the first launch of a checkpoint-bearing
    // kernel that lands on this Handle (i.e. graph_do_while + checkpoint kernels that fall through the graph
    // fast path). Mirrors the per-CachedGraph state in `GraphManager`: `checkpoint_resume_point_dev_ptr` and
    // `checkpoint_yield_signal_dev_ptr` hold device addresses of int32 scalars the codegen prologue and the
    // yield-check kernel dereference. `checkpoint_yield_on_slots[cp]` holds the device address of a slot that
    // carries the user's `yield_on=` ndarray address for cp `cp` (or nullptr if cp `cp` is non-yielding); slot
    // contents are host-updated each launch via memcpy. All three are freed once in the launcher destructor.
 ...
 private:
  ...
  void launch_offloaded_tasks_with_do_while(LaunchContextBuilder &ctx,
                                            ...
                                            void *context_pointer,
                                            int arg_size);
  // GPU-side `qd.checkpoint` setup for the streaming path. Lazily allocates the per-handle resume_point /
  // yield_signal / yield_on slots, updates per-launch state (slot contents, resume_point HtoD,
  // yield_signal init), and wires the device pointers into `ctx.get_context()`. Must be called BEFORE the
  // launch's `memcpy_host_to_device_async` of the RuntimeContext, since that's what publishes
  // `ctx.get_context().checkpoint_*_ptr` to the GPU. No-op if the kernel carries no checkpoints.
  void prepare_streaming_checkpoint_state(LaunchContextBuilder &ctx,
                                          Context &launcher_ctx,
                                          const std::vector<OffloadedTask> &offloaded_tasks);
  // Launch the pre-built AMDGPU yield-check kernel directly via hipModuleLaunchKernel. Used by the streaming
  // path to keep `yield_signal` / `resume_point` consistent with the codegen prologue's view, mirroring how the
  // graph fast path inlines the yield-check kernel into the flat HIP graph.
  void launch_streaming_yield_check_kernel(Context &launcher_ctx, int32_t cp_id, void *stream);
  // Read the device-side `yield_signal` int32 and publish to the GraphManager surface. Mirrors the post-launch
  // D2H the graph fast path does (in `GraphManager::launch_cached_graph`). Returns the read value (`-1` if no
  // yield this launch).
