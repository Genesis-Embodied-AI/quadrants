...
struct AMDGPUConvertAllocaInstAddressSpacePass : public FunctionPass {
  ...
  bool runOnFunction(llvm::Function &f) override {
    ...
    // Force wave64 codegen on every AMDGPU target. RDNA (gfx10+) defaults to wave32 in the AMDGPU
    // backend; explicitly enabling +wavefrontsize64 / disabling -wavefrontsize32 makes RDNA emit
    // wave64 instructions matching CDNA, so we only have to maintain one wave size in the runtime
    // and codegen. Mirrors the libdevice variant pinned in llvm_context.cpp and the TargetMachine
    // features in jit_amdgpu.cpp.
