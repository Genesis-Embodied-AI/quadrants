// Lazy loaders for the three CUDA fatbin-backed kernels that implement `qd.checkpoint(...)` on the
// CUDA-graph path. Extracted out of `graph_manager.cpp` so the main `GraphManager::try_launch` /
// `launch_cached_graph` orchestration stays focused on the standard CUDA-graph build flow; every
// function in this file is checkpoint-specific and only matters when a kernel has at least one
// `qd.checkpoint(...)` block.
//
// Three loaders, all lazy, all idempotent across launches:
//
//   ensure_cond_with_yield_kernel_loaded:
//     `_qd_graph_do_while_cond_with_yield` -- the cond-with-yield variant of the WHILE condition
//     kernel. Required only when the kernel has BOTH `qd.graph_do_while` and at least one
//     `qd.checkpoint(yield_on=...)` inside the WHILE body. Lives in the same fatbin as the regular
//     cond kernel (loaded by `ensure_condition_kernel_loaded`), so this loader just resolves the
//     function symbol from the already-loaded module; it silently no-ops when the module isn't
//     loaded yet so callers can invoke it unconditionally.
//
//   ensure_checkpoint_gate_kernel_loaded:
//     `_qd_checkpoint_if_gate` -- the gate kernel that writes `(cp_id >= *resume_point)` into a
//     conditional handle before each checkpoint's IF body. SM 9.0+ only (CUDA 12.4+ conditional
//     graph nodes are required); on pre-Hopper the gate is intentionally not loaded and
//     `try_launch` falls back to the flat-graph path where the codegen-emitted body-kernel
//     prologue (`codegen_cuda.cpp::emit_checkpoint_gate_prologue`) does the gating from inside
//     each body kernel.
//
//   ensure_checkpoint_yield_check_kernel_loaded:
//     `_qd_checkpoint_yield_check` -- the yield-check kernel that atomic-CASes a cp_id into
//     `yield_signal` and bumps `resume_point` to INT_MAX. Needed on BOTH SM 9.0+ (inside the IF
//     body) AND pre-Hopper (in the flat-graph chain), so the fatbin covers the broader SM range
//     than the gate kernel's fatbin and no SM gate is necessary here.
...
// Lazy load of the cond-with-yield kernel from the same fatbin as the regular cond kernel.
