...
void GraphManager::ensure_condition_kernel_loaded() {
  ...
  const std::string toolkit_base = std::string(windir, 2) + "\\Program Files\\NVIDIA GPU Computing Toolkit\\CUDA";
  if (std::filesystem::is_directory(toolkit_base)) {
    std::vector<std::filesystem::path> cuda_dirs;
    try {
      for (const auto &entry : std::filesystem::directory_iterator(toolkit_base)) {
  ...
  driver.link_add_data(link_state, /*CU_JIT_INPUT_PTX=*/1, const_cast<char *>(kConditionKernelPTX), ptx_len,
                       /*name=*/"qd_cond", 0, nullptr, nullptr);

  driver.link_add_file(link_state, /*CU_JIT_INPUT_LIBRARY=*/4, cudadevrt_path.c_str(), 0, nullptr, nullptr);
