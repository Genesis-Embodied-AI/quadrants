...
void ScalarPointerLowerer::run() {
  std::array<int, quadrants_max_num_indices> total_shape;
  total_shape.fill(1);
  for (const auto *s : snodes_) {
    for (int j = 0; j < quadrants_max_num_indices; j++) {
      total_shape[j] *= s->extractors[j].shape;
    }
  }
  std::array<bool, quadrants_max_num_indices> is_first_extraction;
