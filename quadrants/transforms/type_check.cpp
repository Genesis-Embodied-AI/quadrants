  ...
  void visit(UnaryOpStmt *stmt) override {
    ...
    if (!is_real(primitive_dtype)) {
      if (stmt->op_type == UnaryOpType::sqrt || stmt->op_type == UnaryOpType::exp ||
        ...
        // popcnt / clz always produce a small non-negative count (<= 64), so we normalise the result
        // to i32 across every backend. This matches CUDA libdevice (__nv_popc / __nv_clz both return
        // int) and the AMDGPU SALU bit-count / leading-zero ops (s_bcnt1_i32_b64, s_flbit_i32_b64)
        // which already return i32 in hardware. Without this override, SPIR-V and x64 would return
        // the operand width (e.g. u64 for a u64 input) while CUDA / AMDGPU returned i32, giving the
        // same kernel source different return types per backend. Worse, mid-codegen ret_type mutation
        // on CUDA / AMDGPU bypasses this pass's promotion logic, so a compound expression like
        // `popcnt(x: i64) + i64(1)` would emit `Add(i32, i64)` and trip an LLVM type-mismatch
        // assertion at IR construction time. Setting ret_type here lets type promotion insert the
        // correct cast_value(i32 -> i64) before codegen runs.
