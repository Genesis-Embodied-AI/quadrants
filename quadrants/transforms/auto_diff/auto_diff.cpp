  ...
  if (autodiff_mode == AutodiffMode::kReverse) {
    ...
    if (use_stack) {
      ...
      for (auto ib : IB) {
        ...
        // `replace_local_var_with_stacks` (so the analyzer sees the AdStackAlloca shape, not the alloca-with-store
        // shape promote_ssa_to_local_var emits) and before `make_adjoint` (so the reverse pass is generated
        // against the cleaned forward IR with no spurious push/load scaffolding).
