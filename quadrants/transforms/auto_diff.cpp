...
class ReverseOuterLoops : public BasicStmtVisitor {
 ...
 private:
  ...
  // Swap sibling {Range,Struct}ForStmts pairwise in the container, keeping non-loop stmts at their
  // original positions (they are setup ops whose SSA operands must dominate both siblings and so
  // cannot safely move). The top-level kernel block is handled by `reverse_segments` before this
  // pass, so we only reorder inside nested non-IB blocks here.
  static void reverse_for_loop_order_in_place(Block *block) {
    ...
    for (int lo = 0, hi = (int)for_indices.size() - 1; lo < hi; ++lo, --hi) {
      std::swap(block->statements[for_indices[lo]], block->statements[for_indices[hi]]);
    }
  }
