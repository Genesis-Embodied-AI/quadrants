...
HostMemoryPool::HostMemoryPool() {
  ...
  QD_TRACE("Memory pool created. Default buffer size per allocator = {} MB",
...
void *HostMemoryPool::allocate(std::size_t size,
  ...
  if (!allocator_) {
    QD_ERROR("Memory pool is already destroyed");
  }
...
void *HostMemoryPool::allocate_raw_memory(std::size_t size) {
  ...
  */
  ...
  if (stat.ullAvailVirtual < size) {
    QD_P(stat.ullAvailVirtual);
    QD_P(size);
    QD_ERROR("Insufficient virtual memory space");
  }
...
#endif
    QD_ERROR("Failed to free virtual memory ({} B)", size);
