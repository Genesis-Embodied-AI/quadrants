...
DeviceCapabilityConfig collect_metal_device_caps(MTLDevice_id mtl_device) {
  ...
  // Apple silicon (M1+) and every AMD / Intel discrete GPU we support on macOS run with a 32-wide SIMD group.  Metal
  // exposes `threadExecutionWidth` only on a compiled `MTLComputePipelineState`, not on the device, but every shipping
  // family is 32 so we hard-code it here.  Surfaces via `Program::subgroup_size()` and goes into the fe-ll cache key.
