...
class OffloadedTask {
 public:
  ...
  // `RangeForStmt::checkpoint_id` / `StructForStmt::checkpoint_id`). The GraphManager will consume this in slice 1c
  // to group consecutive same-cp_id tasks under a CUDA IF conditional node. Default `-1` keeps non-checkpoint
  // kernels (every existing user) on the unchanged code path. Serialised into the offline-cache via the `QD_IO_DEF`
  // list below.
  int checkpoint_id{-1};
