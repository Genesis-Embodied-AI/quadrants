...
// Pick the LLVM syncscope for a user-facing atomic op. On AMDGPU, the default System scope makes the backend refuse
// the native single-instruction atomics (`global_atomic_xor`, `flat_atomic_xor`, ...) and fall back to a
...
// `test_reduction_single_i32[arch=amdgpu-5]` to hang. `agent` scope is the largest scope a Quadrants kernel ever needs
// (one kernel = one device; the host only reads results after kernel completion, which forces a full cache flush
// regardless), and it lets the AMDGPU backend emit the native atomics. CUDA/NVPTX and CPU already lower System-scope
// seq_cst atomics to single-instruction hardware atomics, so they keep the System default. SPIR-V backends spell the
// same "Device" scope explicitly in `spirv_codegen.cpp`. See `docs/source/user_guide/atomics.md` for the per-backend
// scope summary.
//
