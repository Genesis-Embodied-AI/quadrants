...
class TaskCodeGenLLVM : public IRVisitor, public LLVMModuleBuilder {
 public:
  ...
  // Clear every fast-math flag on the FP instruction backing `v`, so LLVM cannot reassociate,
  // contract, or substitute approximations (e.g. sqrt -> rsqrt+refine, sin -> libm fast variant).
  // No-op if `v` is not an `llvm::FPMathOperator`. Exposed so non-LLVM-base backends (AMDGPU, CUDA)
  // that override `visit` for specific ops can honor `stmt->precise` consistently. Note:
  // `setFastMathFlags(FastMathFlags{})` only OR's in flags on this LLVM version, so each flag has to
