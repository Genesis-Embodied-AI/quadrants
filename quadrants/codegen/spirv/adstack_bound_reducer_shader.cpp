...
// Physical-Storage-Buffer load of one 32-bit scalar at byte offset `byte_off_u64` from `base_u64`. Mirrors
// the wrapper-struct PSB load pattern in `adstack_sizer_shader.cpp::psb_load_scalar`: `OpConvertUToPtr` to
// a pointer-to-wrapper-struct, then `OpAccessChain` on the `_m0` member, then `OpLoad` with the `Aligned`
// memory-access operand SPIR-V requires for `PhysicalStorageBuffer` reads. Caller passes the byte offset
// directly so the same helper covers the 4-byte-stride f32 / i32 walk and the 8-byte-stride f64 walk
// (issued as two adjacent 4-byte loads at offsets 0 and 4).
...
// Convenience wrapper around `psb_load_u32_at_byte_off` for the f32 / i32 path: byte offset is
// `elem_idx_u32 * 4`.
...
// Assemble a u64 from two adjacent little-endian u32 PSB loads at byte offsets `elem_idx_u32 * 8` and
// `elem_idx_u32 * 8 + 4`. PSB requires `Aligned 8` for a single 8-byte OpLoad; the source ndarray's element
// start is only guaranteed 4-byte aligned (it may follow a u32 in a containing struct), so we issue two
// 4-byte u32 loads and reassemble the u64 in registers. The shifted-OR pattern mirrors `load_arg_buf_u64_ptr`
// above. Returned as u64 (not bitcast) because the caller does its own bitcast to f64 for the comparison.
...
std::vector<uint32_t> build_adstack_bound_reducer_spirv(Arch arch, const DeviceCapabilityConfig *caps) {
  ...
  // f64 gate extension. Only consulted when the device supports `spirv_has_float64`; on devices without f64
  // the host launcher filters f64-captured bound_exprs out of the dispatch (falling back to worst-case heap
  // sizing) and these slots are never read. Loading them unconditionally keeps the shader's static layout
  // matched against the host launcher's params blob writeback.
  ...
  ir.start_label(active_block);
  {
    ...
    // `snode_byte_base_offset + gid * snode_byte_cell_stride` directly into the bound root buffer and load
    // the element word(s)). The element width is 4 bytes for f32 / i32 and 8 bytes for f64; the f64 path
    // issues two adjacent 4-byte loads and reassembles into a u64. We always materialise the loaded value
    // as a u64 (low 32 bits zero-extended in the f32 / i32 case) so the dtype-branch downstream can pick
    // f64 / f32 / i32 reinterpretation without re-loading.
    ...
    {
      // SNode root buffer is a u32[] view. Per-snode-descriptor alignment guarantees `snode_byte_base_offset`
      // and `snode_byte_cell_stride` are multiples of 4. f32 / i32 fields walk one 4-byte word per cell; f64
      // fields walk two adjacent 4-byte words and reassemble into a u64. Issuing two u32 loads (rather than
      // one u64 load) keeps the alignment requirement at 4 bytes so any dense parent's f64-cell layout works
      // without further alignment promotion in the descriptor binding.
    ...
    {
      // ndarray-backed: PSB-load one u32 (f32 / i32) or two adjacent u32 words (f64). The base pointer is
      // assembled from the two arg-buffer u32 words at `arg_word_offset` and `arg_word_offset + 1`.
    ...
    // Branch on `field_dtype_is_float`. The float path further forks on `is_double` to pick f32 vs f64
    // bitcast + comparison; the int path (i32) truncates the u64 to u32 and bitcasts to i32. The f64 inner
    // arm is only emitted on devices advertising `spirv_has_float64`; on f32-only devices the host launcher
    // filters f64-captured bound_exprs out of the dispatch entirely (see adstack_bound_reducer_launch.cpp's
    // matched-task filter), so the inner arm is never reached at runtime - and skipping its emission keeps
    // the OpType for f64 out of the binary, which is what spirv-val requires on f32-only targets.
    Label float_lbl = ir.new_label();
    Label int_lbl = ir.new_label();
    Label dtype_merge = ir.new_label();

    // See note above: `alloca_variable` hoists OpVariable to the entry block; pair with stores on every
    // reachable path through the dtype-branch so the merge-block load never sees undef.
