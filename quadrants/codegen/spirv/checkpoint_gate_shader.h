...
// SSBO bound at slot 1, populated at graph-build time and never written by the GPU.
//
...
//
// Dispatched once per checkpoint at (1, 1, 1). N is small (handful to ~30 body kernels per
// checkpoint), so the single-thread loop fits well inside one workgroup.
//
// Returns the finalized SPIR-V binary; never empty (no capability requirement beyond a vanilla
// compute pipeline). Caller compiles into a `Pipeline` via `Device::create_pipeline`.
std::vector<uint32_t> build_checkpoint_gate_spirv(Arch arch, const DeviceCapabilityConfig *caps);

// Word offsets within the params SSBO (binding 1) the gate shader reads. Layout is fixed; the
// host launcher writes the params buffer once at graph-build time matching these offsets.
...
// Word offsets within the control SSBO (binding 0). Layout is shared with the yield-check shader
// in `checkpoint_yield_check_shader.h`; the two shaders operate on the same per-launch control buffer.
