...
// Builds the SPIR-V compute shader that evaluates a captured `TaskAttributes::StaticBoundExpr` predicate
// over a thread range and atomic-adds 1 into a per-task slot of `BufferType::AdStackRowCounter` for each
// thread that passes. Dispatched once per adstack-bearing task before the main task on the static-IR-bound
// sparse-adstack-heap path; the resulting count sizes the float adstack heap allocation exactly.
//
// The shader is generic (parametrised at dispatch time by the parameter blob in binding 2) and is compiled
// once per `GfxRuntime`. Host responsibility per dispatch:
...
// After dispatch + sync the slot's value equals the number of threads whose `field[i] cmp threshold` matched
// the captured polarity; the host reads that count and sizes the float heap to `count * stride_float *
// sizeof(f32)` before binding the main task.
//
// Required device capabilities: `spirv_has_physical_storage_buffer` + `spirv_has_int64`. The first is needed
// because the gating field is read through the ndarray data pointer the kernel arg buffer carries (PSB load
// path, mirroring the main kernel's ndarray access); the second is needed for u64 pointer arithmetic. On
// devices without either capability the function returns an empty vector and the runtime falls back to the
// dispatched-threads worst-case heap sizing - safe but no savings.
std::vector<uint32_t> build_adstack_bound_reducer_spirv(Arch arch, const DeviceCapabilityConfig *caps);

// Compute-shader workgroup size (x dimension; y and z are 1). Power-of-two and a multiple of typical
// subgroup widths on Metal / Vulkan so atomic-add contention amortises per workgroup. Host launcher uses
// this to compute `num_workgroups_x = (length + kAdStackBoundReducerWorkgroupSize - 1) /
// kAdStackBoundReducerWorkgroupSize` per dispatch.
constexpr uint32_t kAdStackBoundReducerWorkgroupSize = 128;

// Layout of the parameter blob the host writes into binding 2 before each dispatch. POD; keep field order
// in sync with the shader's compile-time word-offset constants in `adstack_bound_reducer_shader.cpp`.
struct AdStackBoundReducerParams {
  // Slot index in the per-kernel `BufferType::AdStackRowCounter` array that the matching atomic-adds will
  // accumulate into. Matches the `task_id_in_kernel` of the main task this reducer is sizing.
  uint32_t task_id_in_kernel;
  // Number of threads to dispatch over (the iteration bound of the gating predicate). Threads with
  // `gl_GlobalInvocationID.x >= length` early-return so dispatch can be rounded up to the workgroup-size
  // multiple without overcounting.
  uint32_t length;
  // u32 word offset into the kernel arg buffer where the ndarray data pointer (u64, two adjacent u32 words)
  // lives. The shader does `OpConvertUToPtr` on that pointer and PSB-loads the gating field's element at
  // `gl_GlobalInvocationID.x`. Only used when `field_source_kind == NdArray`; SNode-backed sources are not
  // yet supported by this shader (the runtime's caller falls back to worst-case sizing on SNode).
  uint32_t arg_word_offset;
  // Encodes the captured `StaticBoundExpr::cmp_op` as an integer: 0 = cmp_lt, 1 = cmp_le, 2 = cmp_gt,
...
// so the host launcher can assign directly from `BinaryOpType` without a static_cast.
