...
#undef UNARY_STD
  ...
  // GPU-side `qd.checkpoint` gating prologue. Emits the LLVM-IR equivalent of:
  //
  ...
  // RuntimeContext slots (e.g. the non-graph CUDA launch path that lands on the same compiled
  // task functions). When the slots are populated (the cached-graph path - both pre-Hopper
  // and SM 9.0+), the check skips the body whenever its cp_id is below the current
  // resume_point or some earlier checkpoint in this launch already set yield_signal. The
  // `final_block` jump matches the existing early-return pattern used by `cpu_assert_failed`
  // gating so all the per-task cleanup (epilogue + return) runs unchanged.
  //
  ...
  // the launch but not already-in-flight bodies). On pre-Hopper CUDA the prologue *is* the
  // gating mechanism since conditional nodes are unavailable.
