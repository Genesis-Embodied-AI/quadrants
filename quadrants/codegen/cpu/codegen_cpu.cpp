...
class TaskCodeGenCPU : public TaskCodeGenLLVM {
 public:
  ...
  void visit(OffloadedStmt *stmt) override {
    ...
    // Propagate qd.checkpoint id off the offloaded stmt onto the codegen task so the CPU KernelLauncher can
    // implement host-branch gating (slice 6). Matches the equivalent lines in `codegen_cuda.cpp` /
    // `codegen_amdgpu.cpp`. Without this assignment every task ends up with `checkpoint_id == -1` ("no checkpoint")
    // and the launcher's resume / yield logic can't tell which tasks belong to which `qd.checkpoint(...)` block.
